...
- 현재 장기개선 goal은 달력 월말이 아니라 사용자에게 배정된 10만 토큰이 초기화될 때까지
  유지한다. 초기화 시 진행 중 검증·배포 체크포인트와 잔여 위험을 기록하고 goal을 종료한다.
- 서비스 변경 회차마다 근거 기록, 전체 테스트, DB 백업, Git 커밋·푸시, 서비스 재시작,
  텔레그램 보고, 운영 상태 재확인을 한 묶음으로 완료한다.
