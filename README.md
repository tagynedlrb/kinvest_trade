...
- **미장 연속손실 차단 후 시장회복 재검사**: 미장 30분 시간차단이 끝나도 같은 뉴욕 거래일의 최신 NASDAQ 관측이 10분 이내이고 수익률이 `-1.0%` 이상으로 회복돼야 일반 롱 신규진입을 재개한다. 회복 전에는 `post_cb_benchmark_not_recovered`, 관측 누락·노후화 시에는 각각 `post_cb_regime_unavailable`·`post_cb_regime_stale`로 대기한다. 인버스 후보와 기존 보유 청산은 계속 평가한다. 이 기준은 미장 정책의 `post_cb_reentry_*` 필드가 소유하며, 직접 사후 재진입 표본이 없는 국장은 현재 비활성이다.
