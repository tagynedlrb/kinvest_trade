...
9. **주문 접수는 체결이나 실현손익이 아니다.** `broker_order_executions`가 KIS 주문체결내역의
   ...
   KIS 매도 체결수량까지 확인하며, 승패도 Gross가 아닌 Net 부호로 판정한다.
...
11. **완전 매도 체결 직후의 stale 잔고로 같은 수량을 다시 매도하지 않는다.** KIS가 잠시
    `보유>0·매도가능=0`을 반환하더라도 최근 5분 내 같은 종목의 실행그룹이 목표수량까지
    완전 체결됐고 체결수량이 stale 보유수량을 덮으면 잔고 반영을 기다린다. 부분체결에는
    적용하지 않으며, 최근 완전체결 증거가 없으면 기존의 실제 청산 시도 경로를 유지한다.
...
13. **KIS REST 호출 페이싱은 클라이언트나 프로세스가 아니라 활성 계정 프로필 전체의
    ...
    실패로 과장하지 않되, 호출 간 경합의 운영지표로 계속 기록한다.
