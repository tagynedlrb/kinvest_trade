...
- **반복 주문거부(`net_profit_below_cost`) 알림 폭탄(2026-07-14)**: 위 CRAN 사건으로 의도치 않게 생긴 5,251주 포지션이 매입가와 현재가가 거의 같아, 시간만료(수익) 청산 조건은 매 사이클 계속 성립하는데 수수료를 반영한 순손익 추정(`_estimate_overseas_net_pnl`)이 0 이하라 매도 자체는 계속 보류(`net_profit_below_cost`)되는 상황이 발생했다. 이 보류는 실제 브로커 주문거부가 아니라 봇이 주문을 내기 전에 스스로 거는 내부 가드인데, `_send_summary`가 이 상태를 매 사이클(요기서는 1분 간격)마다 `동작=주문거부`로 텔레그램에 보내 동일한 메시지가 계속 반복 발송됐다. 매도 보류 판단 자체는 그대로 두고(수수료보다 작은 이익에 파는 건 여전히 막아야 함), 동일한 (시장, 종목, 스킵 이유) 조합의 이런 무주문 알림만 `repeated_skip_notify_cooldown_minutes`(기본 30분) 동안 한 번만 보내도록 억제했다. 실제로 주문이 제출된 매수/매도 체결 알림은 이 억제와 무관하게 항상 즉시 전송된다.
