...
10. **하락장 역방향 상품은 시장별 별도 정책이며 현재는 섀도 전용이다.** 같은 세션의
    **상품별 정확한 기준지수** 하락과 상품 자체의 장중 상승·거래량을 모두 확인한다.
    국장 `114800`·`252670`은 F-KOSPI200, SQQQ는 Nasdaq-100, SPXU는 S&P 500을
...
12. **프로세스 재시작은 리스크 상태나 세션 귀속을 초기화하는 새 거래 세션이 아니다.**
    ...
    되살리지 않고, 그 시각 뒤의 확정 청산만 0부터 다시 센다. 미장 신규 후보의
    신호불가 180분 억제도 유효한 종료시각과 실패횟수를 저장해 배포 재시작이 같은
    차트 호출을 반복하지 않게 하되, 보유종목은 억제하지 않아 청산 감시를 유지한다.
13. **KIS REST 호출 페이싱은 클라이언트나 프로세스가 아니라 활성 계정 프로필 전체의
    ...
    관찰만으로 전체 호출이나 국내 조회를 더 늦추지 않는다. VPS가 멱등 조회 GET에
    `90020000`(모의투자 서비스 지연·재시도 요청)을 반환하면 2초, 4초 뒤 같은
    논리요청 안에서만 다시 조회하고 각 시도를 `service_delay`로 기록한다. 주문 POST는
    브로커 접수 여부가 불명확한 API 오류나 전송 후 응답 단절을 자동 재전송하면
    중복주문이 될 수 있으므로 모든 non-GET은 한 번만 전송하며, 실전 프로필도
    관측되지 않은 모의 응답 규칙의 영향을
    받지 않는다. 자연 제한응답 뒤에는 VPS 프로필별 120초 동안 응답완료 후 950ms
    적응 경계를 사용한다. 이와 별도로 직전 완료 경로와 현재 첫 시도가 모두 정확히
    해외잔고인 VPS 연속쌍만 응답완료 후 850ms 예방 경계를 사용한다. 두 값은 공식
    한도가 아닌 관측 실험값이며 실전·국내·현재가·주문·재시도에는 전파하지 않고,
    실제 추가대기를 각각 분리 기록해 회피한 재시도 비용과 계속 비교한다.
...
16. **인버스 성과는 낙폭뿐 아니라 진입·청산 사이 시장 경로로 평가한다.**
    섀도 진입에는 같은 세션 상품 기준지수의 수익률, 장중 저점 수익률, 저점 이후 반등폭,
    고가-저가 범위 내 현재 위치, 정규장 마감 잔여시간을 고정하고, 청산에는 당시
    레짐과 신호 스냅샷을 추가한다. `/lab_performance`는 최근 세 건의 진입지수,
    저점반등, 범위위치, 마감잔여, 보유 중 지수변화와 비용차감 성과를 함께 보여준다.
    상품별 진행·종료 수와 고유 진입 시장세션 수도 따로 표시해, 같은 하락일에 함께 열린
    여러 ETF 경로를 서로 독립된 시장 표본으로 세지 않는다.
    새 정확지수 행은 `product_exact_v1`으로 표시하고, 예전 KOSPI/NASDAQ Composite
...
- `max_hold_cycles`를 넘긴 뒤에도 총수익이 시장별 왕복 수수료와 0.3%p
  안전 여유를 합친 기준보다 작으면 강제 매도하지 않는다. 이 상태는
  `time_exit_cost_floor_hold`로 구분해 보유기간·진입시각·가상 여부와
  함께 `cycle_log`에 남기며, 비용을 넘는 수익 또는 손실 청산 조건이
  생기면 기존 정책대로 다시 평가한다.
...
- 국내 동적 랭킹의 인버스·레버리지 이름은 정책 승인 목록과 대조한다. 미승인
  구조화상품은 일반 장기 매수 후보에서 제외하고, 승인 인버스도 상품별 F-KOSPI200
  하락 레짐 경로로만 추가한다. 이미 보유한 종목은 이 신규진입 분류와 동적 순위에
  관계없이 현재 사이클 잔고에서 다시 넣어 감시한다.
- 국내 승인 레버리지 롱(`122630`)은 `domestic_momentum_v4`에서 일봉과
...
- 체결확정 기반 동적 전략 가드는 국장·미장을 `(시장, 전략)` 키로 따로 계산한다.
  ...
  독립적으로 고도화한다. `liquidity_lab`은 전체 활성화와 대상 시장 범위만 담당한다.
  한 시장의 손실이나 이후 임계값 변경이 다른 시장을 막지 않으며, 감시대상
  선택과 실제 주문 제출 직전에 모두 재검사한다. 단순 거래별 평균이 양수여도
...
- **연속손실 차단 후 시장회복 재검사**: 30분 시간차단이 끝나도 같은 현지 거래일의 최신 지수 관측이 10분 이내이고 시장별 회복선(미장 NASDAQ `-1.0%`, 국장 KOSPI `-3.0%`) 이상이어야 일반 롱 신규진입을 재개한다. 같은 세션 1회 발동 후 종일 정지는 해당 세션의 체결확정 전략 청산에서 실제 3연속 손실이 확인된 경우에만 적용한다. 전일 손실 이월이나 가상정산 같은 비전략 계좌 청산이 섞여 발동한 경우에는 30분 차단과 지수 회복 검사는 유지하되 종일 정지에는 세지 않고 `post_cb_cross_session_streak_exempted`를 기록한다. 회복 전에는 `post_cb_benchmark_not_recovered`, 관측 누락·노후화 시에는 각각 `post_cb_regime_unavailable`·`post_cb_regime_stale`로 대기한다. 인버스 후보와 기존 보유 청산은 계속 평가한다.
- **장기 가상청산 미체결 가격 전환**: `virtual_sell_pending` 정산은 기존 5분 미체결 취소, 15분 재시도, 현지 세션당 3회 제한을 유지한다. 서로 다른 두 세션 이상 접수 후에도 체결되지 않으면 최신 매수호가와 현재가를 비교해 기준가 대비 최대 50bp 아래의 시장성 지정가(`aggressive_limit`)로 전환한다. 제출가·호가·실패 세션 수를 주문 이벤트와 체결 컨텍스트에 함께 기록한다.
...
- `/lab_portfolio`: 실제 계좌 보유, 전략 유효보유와 보유기간, 실제
  해외·합산 포지션 한도 점유, 정산 대기 매도, 누적 성과 조회
...
같은 화면의 `─── 포지션 한도 ───`는 실보유와 가상보유의 시장·종목
합집합을 실제 신규진입 한도와 비교한다. 실보유에 대한 가상 매도가
`정산대기`인 종목은 전략 유효보유에서는 빠져도 KIS 체결확정 전까지
실제 한도를 계속 점유한다. 따라서 `10/10 가득참`과 `11/10 초과`를
구분해 표시하며, 단순히 매매 빈도가 낮다는 이유로 한도를 늘리지 않는다.
`─── 전략 유효보유 ───`에는 각 종목의 복원 가능한 최초 진입시각 기준
보유기간도 함께 표시한다.
...
- 해외 주문번호는 현지 `ord_dt`와 함께 대조해 날짜별 번호 재사용을 분리한다.
  KIS 주문이력에 해당 주문이 있고, 그 제출 뒤 같은 종목·원주문번호의
  `CANCELED` 취소 이벤트도 있을 때만 취소를 터미널 증거로 적용한다. 이때
  체결수량 0이면 무체결 종결하고, 부분체결은 체결분만 반영한다. 취소 이벤트만
  있고 KIS 주문이력이 없으면 체결·취소·거부를 추정하지 않고 `PENDING`으로
  보존한다. 적용한 후속 이벤트 ID와 사유는 실행행 `history_json`에 남긴다.
...
- 하락장 후보는 국장 `114800`·`252670`, 미장 `SQQQ`·`SOXS`·`SPXU`로 별도 관리한다.
  같은 현지 세션의 **상품별 기준지수**가 -1% 이하이면서 하락 추세이고 상품 자체의
  분봉 추세·모멘텀·거래량이 확인될 때만 후보군에 넣는다. 국내 두 상품은 F-KOSPI200,
  SQQQ는 Nasdaq-100, SPXU는 S&P 500을 쓰며, SOXS는 정확한 NYSE Semiconductor
  Index 소스가 확보될 때까지 후보 진입을 차단한다. 현재 모드는 양 시장 모두
  `shadow`다. `inverse_shadow_trades`
  원장은 보수적 반스프레드와 왕복 수수료, 이익/손실/하드스톱, 시간제한, 지수 회복,
  세션 rollover를 기록하며 `/lab_performance`에서 시장별 섀도 순성과를 함께 보여준다.
  각 신규 진입에는 같은 세션 지수의 장중 저점·저점 이후 반등·고저 범위 위치·정규장
  잔여시간을 저장하고, 청산에는 당시 지수와 신호를 저장한다. 최근 세 건은
  `/lab_performance`에서 진입부터 청산까지의 지수 경로와 함께 표시한다.
  섀도 거래가 0건이어도 시장별 레짐 차단과 상품 게이트 관측을 함께 표시한다. 일일 재설정
  상품이므로 다음 세션까지 이월하지 않는다. 열린 세션의 비확정 상품별 기준지수 레짐은
  5분마다 갱신하지만 정책 성과평가에는 마감 확정값만 사용한다. F-KOSPI200은 KOSPI200
  선물 정규장 종료 15:45 KST 이후인 15:50부터만 최종 처리한다. 활성 인버스와 열린 섀도
...
- `/lab_guard`는 각 시장 정책의 `strategy_guard_lookback_hours` 기간에
  ...
  감시범위 밖 공식이 거래수·평균순손익 임계치를 모두 넘으면
  `참고(범위밖·임계초과)`로 표시해 다음 공식평가 후보로 남긴다.
  현재 미장 정책은 3개 확정 세션에서 비용 후 음수였던
  `VWAP+VOL` 복합 라벨도 감시·차단 범위에 포함하지만, 국장과
  `VWAP+RSI` 등 다른 복합공식에는 이 결정을 전파하지 않는다.
  전략 집계는
...
- 가상매도 수량이 실제 보유 전량을 덮으면 전략 노출은 이미 닫힌
  ...
  종목은 포지션 한도를 계속 점유한다.
- 완전 정산대기 중에는 현재가·잔고·실정산 감시는 유지하되 새
  일봉·분봉 전략 계산은 생략한다. 부분 대기, 같은 종목의 가상보유,
  활성 인버스 관측 대상은 계속 신호를 갱신한다.
- 실정산 지정가는 최소 5분 동안 체결 기회를 준 뒤 취소하며, 미체결
  재제출은 종목별 15분 간격과 뉴욕 세션당 3회 상한을 적용한다.
  제출 횟수는 DB 주문 이벤트로 계산해 서비스 재시작으로 초기화되지
  않는다. 체결내역 연속조회가 50페이지에서도 끝나지 않으면 일부
  원장만 확정하지 않고 진단 이벤트를 남긴다.
...
### 시장이 모두 닫혀 있을 때
국내장과 미국장(애프터마켓 포함)이 모두 닫혀 있는 시간대(KST 07:00~09:00, 15:30~17:00 부근)에는 프로그램이 API 호출 없이 대기 상태로 유지된다. 단, 효력 발생한 미장 기업행동의 가상 포지션이 남아 있으면 다음 장까지 오래된 포지션을 유지하지 않도록 현재 사이클 KIS 잔고를 한 번 대조하고, 정산 뒤 다음 폐장 사이클부터 다시 API 0회 대기로 돌아간다. 다음 거래 가능 세션이 임박하면(30분 이내) 감시 주기가 짧아지고, 그렇지 않으면 길어진다(`determine_loop_interval_sec` 참고). 이때 KRX/NYSE 휴장일은 다음 거래 가능 세션 계산에서 건너뛰며, 미국장은 KIS의 한국시간 기반 주간/프리/정규/애프터 세션 날짜에 맞춰 휴장일을 판단한다.
...
- **해외 미체결 조회 페이지 누락 버그(2026-07-13, CRAN 사건)**: 해외 주문내역 조회(`get_overseas_order_history`)를 모의투자(`vps`) 환경에서 전종목·전체 상태로 한 페이지만 호출해 당일의 무관한 이력이 결과창을 채웠다. 실제 미체결 주문이 조회 결과에서 사라지면서 중복방지 로직(`_find_open_overseas_order`)이 "없음"으로 오판했고, CRAN 한 종목에 30분 동안 60건 이상의 매수주문이 중복 제출됐다. 당시에는 종목별 `symbol`과 `fill_filter="02"`로 우회했지만, 이후 KIS 공식 샘플을 대조하니 모의투자는 빈 `PDNO`·거래소와 `00` 전체 매매/체결 조건만 지원한다고 명시돼 있었다. 현재는 공식 조건과 뉴욕 현지 당일을 사용해 연속페이지를 모두 모은 뒤 `nccs_qty > 0`만 로컬에서 선별한다. 스냅샷은 한 서비스 사이클 동안 종목들이 공유하고 실주문 제출·취소 직후 무효화한다. 실전계좌의 전용 미체결 API와 과거 체결 조정은 별도 경로를 유지한다.
...
- **체결 직후 양수 잔고 재노출로 중복매도·허위 주문거부(2026-08-12)**: 최근 7일의 `40240000 잔고내역 없음` 매도거부 3건은 모두 직전 매도 접수 63~143초 뒤 KIS 잔고가 일부 또는 전량을 다시 주문가능으로 보여 재매도한 경우였다. 실시간 미체결 목록에는 이미 사라졌지만 로컬 체결원장은 아직 조정 중인 짧은 공백이었다. 국내·해외 매도는 미체결이 없더라도 최근 접수 매도가 표시 잔고를 덮으면 최대 8분, 최근 전량체결이면 5분 동안 재주문을 보류한다. 브로커에 실제 미체결이 보이면 기존 보호성 45초/일반 15분 정정 규칙은 유지한다. 같은 날 `watch:` 정책관망 18건이 `동작=주문거부`로 발송된 별도 오분류도 수정해, 정책관망은 빈도 로그에만 남기고 내부 스킵은 `매매미실행`, 실제 API 거부만 `매수거부`/`매도거부`로 표시한다.
- **가상정산 주문 폭주가 체결원장까지 가린 복합 장애(2026-08-14~17)**: NPAC 정산 지정가를 45초마다 취소하고 제한 없이 다시 내면서 하루 73회 제출·72회 취소가 발생했다. 해외 주문체결내역은 208행·14페이지로 늘었지만 리컨실러는 10페이지만 읽고 잘림을 알리지 않아, 실제 체결된 일반 매매까지 포함한 51행이 계속 `PENDING`으로 남았다. 읽기 전용 KIS 전수조회와 DB 복제 검증으로 51개 주문번호를 모두 확인한 뒤, 체결조회 상한을 50페이지로 늘리고 잘림 감지, 이전 세션 DAY 주문 잔량 종결, 정산 5분/15분/세션 3회 제한을 함께 적용했다. 복구된 미장 손실 때문에 `VOL+RSI` 전량 재개도 실패로 재평가했다. 미장 가드는 이제 3개 최종세션 외에 차단 후 소규모 거래 3건의 평균 순수익이 0%를 넘어야 해제되며, 국장 정책은 별도 설정으로 기존 해제 규칙을 유지한다. 상세 수치와 검증조건은 `docs/EXECUTION_RECONCILIATION_REVIEW_2026-08-18.md`에 기록했다.
