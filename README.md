...
현재 기본 진입점 `python3 main.py` 는 해외주식 이동평균 자동매매 루프를 바로 실행하도록 연결되어 있다.
...
현재 기본 정책은 `OVERSEAS_MA_BAND` 이다.
- 일봉 `20일선/60일선`과 `5분봉 5선/20선`을 함께 사용한다.
- flat이면 `60일선 근처 + 5분봉 상향 전환`에서 진입하고, 이미 `20일선 위 추세 복귀 + 5분봉 상향 교차`가 확인되면 추세 재진입도 허용한다.
- 보유 중에는 `20일선 하향 이탈`, `60일선 실패`, `과열 구간`, `트레일링 되돌림`을 나눠서 판단한다.
...
### 전략 핵심
- 진입은 `20일선 위 추세 복귀` 또는 `60일선 근처 반등` 2갈래로 나뉜다.
- 손절은 고정 퍼센트만 쓰지 않고, `5분봉 변동성 * 배수`와 `20일선/60일선 이탈 폭`을 함께 본다.
- `20일선 재접근` 시에는 절반 익절이 가능하고, 이후 추세가 유지되면 잔여 수량은 홀딩한다.
- `max_position_qty`, `allow_scale_in`, `allow_partial_exit`로 보유 수량을 고정하지 않고 가변적으로 조절한다.
- `poll_interval_sec=20`으로 시세는 자주 감시하되, 무거운 차트 컨텍스트는 `5분봉 기준`으로 캐시해 호출 수를 억제한다.
...
- 해외 기본 제외 기준은 `10달러 미만`, `거래량 5만 미만`, `스프레드 0.4% 초과`다.
- 현재 장이 열린 시장에서 `activity_score`가 가장 높은 1개 종목을 기본 `primary_target`으로 선택한다.
- 다만 실제 해외 주문은 `activity_score`만으로 바로 넣지 않고, 선택된 후보가 `이동평균 진입 조건`을 만족할 때만 진행한다.
- 다만 해외 mock 포지션이 이미 있고 손절/익절 기준에 먼저 걸린 보유분이 있으면, 신규 매수보다 기존 보유 청산을 우선한다.
- 고정 손절/익절에 먼저 걸리지 않았더라도, 보유 종목이 `20일선/60일선 이탈` 같은 이동평균 청산 신호를 보이면 청산 후보로 올린다.
