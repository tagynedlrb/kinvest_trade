...
### 성적과 정책 판단
- 최근 14일 계좌 확정 청산은 국장 15건·20% 승·평균 Net -0.142%·-3,119원,
  미장 4건·0승·평균 Net -0.532%·-23,645원이다.
- 9월 21일 v9/v6 재시작 뒤 새 체결은 0건이다. 주문거부나 주문경로 장애가 아니라
  완전한 진입식 통과가 0건이며, READY 13회는 모두 `near_breakout` 대기였다.
- 국장 진입확인 차단군은 3세션·26~33표본의 5~120분 전 구간이 비용 후 음수다.
  미장 `volume_low`도 상승·하락·보합·급등 네 확정 장세의 15/30/60분 비용 후
  성과가 전부 음수라 진입 기준 완화를 거부했다.
- CB 차단 45분은 20표본·3세션에서 기계적 후보지만 세션 중앙이 +0.001%이고
  진입/청산 효과가 섞여 있어 실거래 승격을 보류했다.

### 수정
- 국장에만 있던 고정 보유시간 원장을 시장 공통 관측기로 일반화했다. 정책 ID와
  비용식은 계속 분리하며 미장은 왕복수수료+SEC fee, 국장은 상품별 거래세 면제를 쓴다.
- 미장 `near_breakout_wait`와 주 병목 `entry_volume_blocked`,
  `entry_trend_blocked`를 5~120분 전향 모의군으로 기록하고, 열린 미장 실험 종목이
  통합 감시 상한 밖으로 밀려도 만기까지 보존한다.
- 미장 정규장 중 TradingView 풀이 30개에서 1~3개로 축소된 병목을 확인했다.
  1.8과 1.08 스캔이 목표의 30% 미만일 때 탐색 전용 0.60 스캔을 추가했다.
  실조회에서 2→3→14개로 복원됐고 실제 v6 거래량 확인 2.0은 변경하지 않았다.
- 거래가 0건인 날 진입환경 연결률을 100%, 승률을 0%로 보이던 오류를 `n/a`로
  수정하고 `market_session_review_v2`에서 무분모 품질값을 null로 저장한다.

### 운영 감사
- 서비스는 system scope가 아닌 `systemctl --user`에서 PID 2168268로 정상 실행,
  재시작 0회, runtime `last_error=null`이었다.
- 배포 뒤 API 논리요청 115,809건은 전부 GET이며 종단실패 100건(0.0863%),
  주문 이벤트·주문거부·미종결 실행은 0이다. Telegram 수신 단절 12회는 모두
  복구됐고 발신은 62성공·1실패였다.
- DB `quick_check=ok`, FK 위반 0, 실행원장 1,051행 전부 종결·잔여수량 0이다.
- 최종 전체 908개 테스트(125.64초), `compileall`, 정책 JSON, Ruff 치명검사,
  `git diff --check`를 통과했다.
- 변경 전 온라인 백업은
  `data/trading_backup_20260925_150539_pre_frequency_observability_deploy.db`,
  647,405,568바이트, mode 600, SHA-256
  `f45f72c84554fb023dcf038c48b5712366fdbf901dd91285de1f5fe5d99644a3`,
  `quick_check=ok`, FK 위반 0이다.
- 상세 근거와 반증조건은
  `docs/RUNTIME_PERFORMANCE_AND_FREQUENCY_REVIEW_2026-09-25.md`에 보존한다.

## [2026-09-21] 손실 지속·진입식 우회·조회안전·Telegram 수신장애 재감사

### 성적과 시장 판단
- 최근 14일 계좌 확정 청산은 국장 34건·26% 승·평균 Net -0.208%·-47,715원,
  미장 5건·0승·평균 Net -0.561%·-100,598원이다.
- v8 국내 검증 15청산은 평균 -0.142%, 중앙 -0.210%, 자본가중 -0.139%,
  -3,119원이고 5세션 모두 포함한다. v5 해외 검증 4청산은 0승·평균 -0.532%·
  -$17.52다.
- KOSPI 상승·급등일과 Nasdaq 상승·활발일에도 비용 후 손실이어서 약세장 단독 원인과
  단순 빈도 확대를 거부했다. 최근 진입의 시장환경·현지일 연결률은 100%다.

### 원인과 수정
- v8 `VWAP+VOL` 15진입은 정식 공식 재생 시 `trend_down` 14,
  `near_breakout` 1로 전부 탈락했다. v5 `VWAP+RSI` 3진입도 전부
  `volume_low`였다. 국장 v9·미장 v6에서 각각 실제 복합전략을 확인식에 추가했다.
- 국장 검증 중 거래세 상품 6건은 평균 -0.356%·-2,911원, 면제상품 9건은 거의
  손익분기(-209원)였다. 국내 검증을 거래세 면제 상품으로 제한하고 세션 유효/제출을
  2/4로 축소했다. 차단 기회는 보유시간 shadow로 계속 측정한다.
- MG가 미국장 마감 15.2분 전 진입해 모의 장후청산 불가·주말 이월된 사례를 반영해
  미장도 정규장 마감 60분 전 신규진입을 중지한다.
- 종목 연속손실 쿨다운을 5회 24시간, 8회 7일로 확장했다. runtime의 229200
  13연속손실도 재시작 첫 사이클에 전체 체결원장을 다시 대조해 새 만료시각으로
  복원한다. 저장된 횟수가 있으면 재대조를 생략하던 배포 검증 중 결함도 수정했다.
- 국내 미체결 조회 실패가 빈 목록으로 바뀌던 결함을 수정해 매수·매도 모두
  `open_order_lookup_failed`로 보류한다.
- Telegram 수신은 webhook 잔존으로 409가 발생해 5,363회·45.9시간 중단됐다.
  webhook을 pending update 보존 방식으로 제거했고, 상태코드·원인을 보존한 자동
  `deleteWebhook(false)` 복구를 추가했다.

### 원장·안정성
- 최종 전체 회귀 테스트 `904 passed in 131.38s`, `compileall`, 정책 JSON 파싱,
  `git diff --check`를 통과했다.
- 배포 이후 POST 36건은 전부 단일시도 성공, 실패·재전송·주문거부·미종결 실행은 0이다.
  GET 실패 2,738건은 주로 KIS ReadTimeout이며 09-16·09-18에 집중됐다.
- broker 실행원장 잔여수량 0, FK 위반 0, 운영 DB `quick_check=ok`, systemd
  `active/running`, `NRestarts=0`, runtime `last_error=null`을 확인했다.
- 변경 전 백업은
  `data/trading_backup_20260921_174816_pre_v9_v6_policy_evaluation.db`, mode 600,
  SHA-256 `15d076bc70f42f01898620d6abd64baba80db1578fc075ed66bdd390d71d2317`,
  `quick_check=ok`이다.
- 상세 분석·반증조건·공식 근거는
  `docs/LOSS_RUNTIME_AND_POLICY_REVIEW_2026-09-21.md`에 보존한다.

## [2026-09-12] 손실 지속·마감진입·가드 조기해제·주문 재전송 재감사

### 성적과 판단
- 최근 14일 세션소유 확정 체결은 국장 58매수·58청산, 미장 7매수·7청산이다.
  계좌 확정 청산 기준 국장은 19% 승·평균 Net -0.119%·-61,355원, 미장은
  38% 승·평균 Net +0.089%이나 금액 합계 -57,713원이다.
- 국장 v7 배포 뒤 국내 기초자산 `VWAP+VOL` 18건은 -41,972원이었다. 이 중
  마감 60분 이내 4건이 0승·-47,308원이고, 120분 이상 남은 14건은
  6승·+5,335원이었다. 최근 손실은 시장 방향만이 아니라 늦은 진입에 집중됐다.
- 미장 v4는 회복거래 4건만으로 가드가 해제됐고 직후 정상 비중 SUNB가 -$57.00을
  기록했다. 검증진입 5건은 평균 +0.364%·자본가중 +0.252%지만 중앙 -0.517%라
  정상 비중 복원 근거가 부족하다.

### 수정
- 국장 `domestic_momentum_v8`: `RSI`, `VWAP+VOL`을 정상 슬롯 10% 검증진입으로
  고정하고 세션당 유효 4회·제출 8회로 확대했다. 14:30 KST 이후 신규 진입만
  `entry_too_close_to_regular_close`로 막고 기존 보유 청산은 유지한다.
- 미장 `overseas_momentum_v5`: `VWAP+RSI`를 세션당 1회·제출 2회·10% 검증진입으로
  고정했다. 국장 마감 제한과 횟수는 복제하지 않았다.
- 마감차단 기회는 `market_close_blocked` 보유시간 모의군에 저장한다. 검증 제한의
  기회비용도 이후 확정 세션별로 평가한다.
- 09-08 해외 매도 POST 응답 단절 뒤 같은 요청이 재전송된 위험을 수정했다. GET 조회만
  최대 3회 재시도하고 주문·취소 등 non-GET은 한 번만 전송한다.
- 진입 판단은 시장레짐·CB·기업행사·시간 같은 공식 가드를 전략 검증보다 먼저 평가해
  실제 대기 원인을 로그에 남긴다. 검증 한도·지표 노후화·환경 차단도 상세 사유로 기록한다.

### 원장·검증
- 정책평가 121·122를 `falsified_and_superseded`, 123을 8천 회 이상 자연 사이클,
  재시작 0, runtime 오류 0, warning 0 근거로 `confirmed` 처리했다.
- 새 전향평가 127(POST 단발 전송), 128(국장 v8), 129(미장 v5)를 생성했다.
  분석기 기한경과 미검토는 0건이며 시장·진입·섹터 로그 연결률은 100%다.
- 변경 전 백업은
  ...
  `quick_check=ok`이다.
- 구현 커밋 `377a00e4d198691dd594dbcf3daaa8747145adec`, 전체 898개 테스트와
  추가 마감차단 모의군 회귀 테스트가 통과했다.
- 상세 근거와 반증조건은
...
### 성적과 원인
- 2026-09-07 16:07 UTC 기준 최근 14일 세션소유 확정 체결은 국장
  54매수·53청산, 미장 18매수·18청산이다.
  계좌 기준 국장은 53청산·13% 승·평균 Net -0.097%·-27,276원, 미장은
  22청산·27% 승·평균 Net -0.382%·-210,301원이다.
- 9월 7일 KOSPI는 6,995.39, +4.608%, 거래량20일비 0.728의 급등장이었지만
  국장은 13진입·12청산·2승·-22,776원이었다. 거래 부족이나 하락장만의 문제가
  아니라 전략·상품 선택 문제가 확인됐다.
- 국장 v7 전향 범위(`RSI`, `VWAP+VOL`, 국내 기초자산)는 전체 45청산·
  +33,735원이고 나머지는 195청산·-337,715원이다. 다만 전향 범위도 절사평균과
  중앙값이 음수라 검증된 수익 공식으로 과장하지 않고 제한된 전향 실험으로 둔다.
- KRX 상장 해외 기초상품은 9월 11청산·0승·-12,904원이었다. KOSPI 레짐으로
  미국 기초자산을 판단하지 않도록 신규 진입만 차단하며 기존 보유 청산은 유지한다.

### 수정과 보류
- 국장 정책 `domestic_momentum_v7`은 `RSI`, `VWAP+VOL`, 그림자 `INV`만,
  미장 `overseas_momentum_v4`는 `VWAP+RSI`, 그림자 `INV`만 신규 진입시킨다.
- 국장 성과가드 검증진입이 10%로 줄지 않던 결함을 수정했다. 9월 7일 `004310`
  `VOL`은 108주 전량 뒤 -10,797원이었고 검증 귀속도 없었다. 이제 수량·주문사유·
  실행 컨텍스트·이벤트가 미장과 동일한 검증 경로를 사용한다.
- CB 2회 세션 한도와 현행 청산시간은 유지한다. 첫 CB 즉시 중지는 9월 3일 큰
  수익도 제거하며, 5~120분 현행신호 모의군에는 강건한 양수 구간이 없다.
- 인버스는 국내 9청산 평균 -0.472%, 해외 6청산 평균 -0.906%라 그림자 전용이다.

### 운영·기록
- 9월 3일 Telegram 장애 보고 실패가 사이클 task에서 빠져나와 서비스가 한 번
  재시작된 경로를 수정했다. 백그라운드 전송, 완료 task, 스케줄러 회차를 격리하고
  빈 예외도 클래스 이름으로 기록한다. 배포 커밋·dirty·시장별 policy id를 시작
  로그와 runtime에 보존한다.
- 미국 노동절 휴장 중 `us_regular` 상태를 보낸 오류를 찾아 상태 판정에도 휴장
  달력을 적용했다. 휴장 알림 실패는 다음 사이클에 재시도한다.
- 정책평가 108~120의 기한경과 미검토 12건을 실측 결과로 닫고 후속 121~126을
  생성했다. 누락됐던 평가 117의 구현 커밋 연결은 당시 배포 기록과 Git 이력으로
  `6393322`를 복구했다. 분석기 기한경과 경고는 0건이며 시장·진입·섹터 로그
  연결률은 100%다.
- 변경 전 온라인 백업은
  `data/trading_backup_20260907_154628_pre_v7_policy_evaluation_deploy.db`,
  647,405,568 bytes, mode 600, SHA-256
  `018d4c92e4b49747f39249d1da4203f96f29ca8dc64d09e965b32bcec9d41bae`,
  `quick_check=ok`, 외래키 위반 0이다.
- 배포 직전 실계좌 대조에서 국내 `379810` 36주(-720원), 해외 `CCRN` 281주
  (+$4.215), 양 시장 미체결 0건을 확인했다. 체결 실행 971건은 미종결·수량오류·
  체결가누락·이벤트/사이클 고아가 모두 0이다. 9월 7일 장 막판 보유시간 그림자
  61건은 목표시각이 장 종료 뒤여서 다음 KRX 세션에 만료 처리될 정상 장중관측
  대기이며 성과 집계에는 포함되지 않는다.
- 소유자 외 접근 가능했던 구형 8월 28일 백업 1개의 mode를 600으로 교정했고,
  보호 대상 경로 재검사에서 group/other 접근 가능 파일은 0개다. 전체 894개 테스트
  (122.28초), `compileall`, 정책 JSON 파싱, Ruff 치명검사, `git diff --check`가
  통과했다.
- 상세 근거와 반증조건은
...
### 판단
- 2026-07-15 이후 국장 확정 청산 201건을 최종 KOSPI 방향과 결합했다. 급락 40건은
  평균 -0.720%·-269,311원, 하락 31건은 -0.446%·-125,355원인 반면 급등
  98건은 +0.124%·+138,670원이었다. 하락장에서 일반 롱을 쉬되 전면 중단은 하지 않는다.
- 2026-07-27 이후 진입 시 KOSPI 당일 범위 하단 25% 미만 46건은 6승, 평균
  -0.420%, 절사 -0.492%, 중앙 -0.553%, -155,011원이었다. 8월 27일 손실 6건도
  모두 범위 위치 6.4~20.2%의 장중 반전 구간이었다.
- 최근 단독 VOL 20건은 6승, 평균 -0.141%, 자본가중 -0.074%, -11,799원이다.
  시장환경뿐 아니라 단독 신호 품질에도 문제가 있다고 판단했다.
- 5~120분 고정 보유시간은 어느 구간도 비용 후 평균·10% 절사평균·중앙값이 모두
  양수가 아니었다. 45분·120분 일괄 연장과 손절 지연은 거부한다.

### 구현
- 국장 정책을 `domestic_momentum_v6`으로 올렸다. KOSPI 음수 진입 하한은 유지하고,
  당일 범위 위치 25% 미만에서 일반 롱을 중지한 뒤 50% 이상 2회 연속 관측에 재개한다.
- 단독 `VWAP`, `VOL`은 전체 진입 공식을 다시 확인한다. 성과가드 해제에는 발동 후
  양의 회복성과를 요구하고, 모든 가드 전략은 세션당 2진입·4제출·10% 슬롯 검증 경로를 갖는다.
- 확정 손실 3연속 CB는 유지하되 첫 발동은 냉각과 KOSPI 0% 이상 회복 뒤 재개 가능,
  같은 세션 두 번째 발동부터 세션 중지로 조정했다. 인버스는 일반 롱 중지에서 제외한다.
- `entry_horizon_shadows` 원장과 5·10·15·30·45·60·90·120분 모의군을 추가했다.
  ETF 계열은 매도세를 제외하며 열린 실험 종목은 기존 감시 슬롯 안에서 끝까지 추적한다.
- `/lab_report horizon [일]`과 `--horizon-shadow-days` 분석 경로를 추가했다.
- 과거 확정 매수 68건을 온라인 백업 뒤 544행으로 백필했다. 성숙 440, 만료 104,
  중복 0이며 기존 체결·손익 원장은 수정하지 않았다.

### 검증·보류
- 전체 882개 테스트, `compileall`, JSON, `git diff --check`, DB 무결성·외래키 검사가 통과했다.
- 배포 전 서비스 기동 이후 KIS 종단실패·미종결 주문·주문거부는 모두 0이다.
- 국내 인버스 그림자는 8건·2승·평균 -0.622%라 실거래 승격을 계속 보류한다.
- 상세 수치와 반증조건은
  `docs/DOMESTIC_STRATEGY_STOP_RESUME_AND_HORIZON_REVIEW_2026-08-28.md`에 보존한다.

## [2026-08-26] 정산 귀속·탐색성과·무체결 전수감사

### 데이터 오류와 수정
- 가상청산 뒤 실계좌 정산 단계에서 전략 귀속이 끊겨, 체결확정 정산 21건과
  주문 실행/이벤트 각 119건, NPAC 정산대기 1건의 전략·진입 메타데이터가
  비어 있었다. 운영 DB 온라인 복사본에서 확인한 매수 체결·진입가·진입시각으로
  21/119/119/1건을 멱등 복원했고 관련 빈 값은 모두 0건이 됐다.
- 매수 체결 원장이 없는 구형 NPAC 보유는 `lab_symbol_state`의 `VWAP+VOL`,
  `VWAP`, 진입시각을 사용하되 진입사유를 만들지 않고
  `legacy_position_state`로 출처를 표시한다.
- 신규 가상청산은 pending→주문 이벤트→실행 원장→부분/최종 정산까지
  `strategy_flag`, `entry_by`, `entry_reason`, `entry_time`을 보존한다.
- `/lab_guard`에 검증진입의 체결·청산·승·평균/중앙/자본가중 순성과와
  진입 당시 최종 시장 레짐을 추가했다. 최근 336시간 검증진입은 9체결·9청산·
  2승, 평균 +0.056%, 중앙 -0.527%, 자본가중 +0.090%, 누적 +$6.19다.
  두 번의 약 +2% 승리가 전체 양수를 만들고 나머지 7건은 손실이라 확대하지 않는다.
- WAIT 선행성과에 10% 절사평균과 양수 스프레드 관측 비율을 추가했다.
  KIS 모의 해외 시세는 bid/ask를 주지 않아 전 표본 `스프레드>0=0`이며,
  미관측 스프레드 0을 타이트한 호가로 오인해 후보점수 +1을 주던 오류도 제거했다.

### 빈도·시장 판단
- 17:02 UTC Nasdaq은 -0.357%, `down|unknown|calm` 임시 레짐이다. 336시간
  주요 WAIT 버킷은 평균뿐 아니라 절사평균·중앙값까지 0.50% 비용하한을 넘지
  못했다. 일반 가드/거래량/포지션 한도 완화는 거래 수만 늘리고 기대값이 음수라
  거부하며, 비음수 최신 지수에서만 기존 10% 검증진입을 유지한다.
- 국장은 KOSPI +0.971%, `up|normal|calm`, 진입 0·청산 6·순손익
  +5,912.50원이다. 교차세션 연속손실 오귀속 수정은 재생 검증됐고 다음 KRX
  세션의 실제 진입 경로가 전향 검증 조건으로 남는다.
- 인버스 그림자는 국내 8건 평균 -0.622%, 해외 5건 -0.926%라 실거래 승격을
  계속 보류한다.

### NPAC 반복 미체결
- NPAC 396주는 8거래일 동안 정산 실행 94건이 모두 0체결로 취소·종결됐고,
  8월 26일 50bp 공격지정가 $10.4136도 세 차례 미체결이었다. KIS 시세는
  $10.4659, bid/ask 없음, 당일 거래량 0이다.
- 2개 이상 실패세션 뒤 현재 거래량도 0이면 당일 무효 주문 반복을 중단하고
  Telegram/event를 한 번 남긴다. 거래량이 양수로 회복되면 기존 제한가 경로를
  자동 재개한다. 호가가 없는 상태의 무제한 시장가 전환과 원장 삭제는 거부한다.
- Nasdaq 종목 페이지와 SEC 2026년 6월 8-K는 NPAC 상장을 확인하므로 CCRN식
  현금합병으로 임의 정리하지 않는다.

### 감사 결과와 근거
- `quick_check=ok`, FK 위반·실행/이벤트 고아·수량 불변식 위반·미종결 실행·
  체결그룹 누락은 모두 0이다. 배포 후 8,183 API 시도 중 회복 재시도 1,
  종단실패 0이며 Telegram 최근 7일 309건은 모두 성공했다.
- 영향 모듈 234개, 신규 핵심 회귀와 전체 **875개** 테스트(121.52초),
  `compileall`, 설정 JSON, `git diff --check`가 통과했다.
- 배포 전 온라인 백업은
  `data/trading_backup_20260826_170710_pre_settlement_attribution_deploy.db`,
  647,405,568바이트, mode 600, SHA-256
  `3ec1b4a532ffd71797f3f4636ec9b43eb99028959d258253a71af925a7609504`이며
  `quick_check=ok`, FK 위반 0이다.
- 1차 구현 커밋 `4acc710908941b7c3f42381a4bedc0a69a659b6a`를
  `git_token.txt` 일회성 인증으로 원격 `master`에 푸시했고 원격 해시가 일치한다.
  17:08:09 UTC 재시작에서 실제 운영 원장도 21/119/119/1건을 복원했으며
  빈 귀속은 0이다. 첫 자연관측은 34개 상태행, API 89시도·88논리요청,
  회복 재시도 1회·종단실패 0이다. 구조화 평가 `id=118`에 근거와 반증조건,
  커밋을 연결했다.
- 첫 자연관측에서 오늘 제출한도 판정이 거래량 0의 근본원인보다 먼저 끝나는
  것을 확인했다. 반복 미체결·거래량 0 판정을 제출한도보다 앞세워 같은 날에도
  정확한 보류 원인과 Telegram을 한 번 남기도록 보정했고 전체 **875개** 테스트를
  다시 실행해 121.08초에 통과했다.
- 보정 배포 전 온라인 백업은
...
## [2026-08-26] 국장 저빈도·신호계산·서비스 전수감사

### 원인과 판단
- 8월 26일 국장 진입 0건은 전일 손실 2건과 당일 첫 손실을 합친 CB 발동이
  당일 3연속 손실로 잘못 귀속돼 종가까지 신규진입을 막은 결과다. 앞선 배포의
  같은 세션 확정손실 귀속 수정은 논리상 해결했으며 다음 KRX 세션 검증이 남았다.
- 운영 DB를 격리 복제해 8월 26일 00:32 UTC 해제 직후를 재생한 결과, 새 코드는
  fire 1건을 같은 세션 손실 0건·교차 세션 1건·당일 연속손실 1건으로 분류했고
  `session_stop_exempted=true`, 최종 차단사유 없음으로 신규진입 경로를 복구했다.
- 8월 19일·24일 0건은 KOSPI -5.803%·-3.124% 강한 하락장에서 일반 롱을
  차단한 의도된 결과다. 8월 전체 국장 121진입·123청산·35승·-213,634원이므로
  거래량/포지션/주문 한도의 일괄 완화는 거부하고 거짓 차단만 제거한다.
- KIS 최신순 봉을 오래된 방향으로 평활하던 RSI 오류와, 전일 봉까지 당일 VWAP에
  섞던 오류를 발견했다. 실데이터 102110 RSI는 62.644에서 39.739, 001210은
  50.196에서 44.848로 교정된다.
- 인버스 실거래 승격은 국내 8건 평균 -0.622%, 해외 5건 평균 -0.926%라 보류한다.
  손실 전 양의 MFE가 반복된 근거로 국내 +0.50%, 해외 +0.60% 비용 후 고점에서
  활성화되는 0.30% 후행 이익보호를 그림자 정책에만 추가했다.
- 미장 최근 336시간 자본가중 순성과도 `VOL+RSI` -0.484%, `VWAP+VOL`
  -0.131%, `VWAP+RSI` -0.679%로 음수다. 활성 가드와 제한적 검증진입은 유지하고
  주문 수를 위한 광범위한 가드 해제는 거부한다.
...
- 기존 프로세스가 `trading.db` FD 52개를 지속 보유하는 것을 확인했다. Python sqlite
  context가 commit/rollback만 하고 close하지 않는 동작을 보완해 모든 저장소 트랜잭션이
  종료 시 연결을 명시적으로 닫는다. 격리 프로세스 500회 연속 조회 뒤 열린 DB FD는 0개다.
- 런타임 의존성을 `requirements-runtime.lock`과 `.venv`로 격리하고 `data/`, `logs/`,
  `state/`만 쓰도록 systemd 속성을 선언했으며,
...
- 배포 직후 `httpx` INFO가 Telegram 요청 URL을 기록하는 것을 포착해 서비스를 즉시
  ...
  이상만 기록하도록 분리했다. 추가 logging 필터·formatter가 일반 메시지, 인자,
  예외 traceback의 Telegram URL과 Bearer 값도 출력 직전에 치환한다.
...
- 실제 거부 쓰기 probe에서는 user systemd의 mount 기반 `ProtectHome/ProtectSystem`이
  이 호스트에서 강제되지 않았다. 실패한 bubblewrap 시험 패키지는 제거했고 sudo 우회는
  거부했다. 현재 확인된 커널 보호는 `NoNewPrivs=1`, 디렉터리 700, 비밀·DB 600이다.
- 상세 수치, 거부 결정, 반증조건은
...
- 문서화 중 14:01:51 UTC의 세 번째 NPAC 주문(execution 836)도 같은 $10.4136에
  정상 접수됐으나 0체결이었고, 14:07:06 UTC에 396주 전량 자동취소·종결됐다.
  SEC 6월 공시는 NPAC을 Nasdaq 상장 Class A 보통주로 표시하므로 CCRN과 같은
  상장폐지 근거는 없다. KIS 매수호가 0에서 가격발견 없이 50bp 상한을 넘겨 추격하거나
  시장가로 바꾸지 않고, 대기 396주와 다음 세션 검증을 보존한다.
...
### 미장 저빈도 진단의 원인 분리
- 14:07:30 UTC 이벤트 `17580`은 최근 미장 주문가능 50사이클 접수 0건을 경고했다.
  원시 사유는 NPAC `virtual_sell_pending` 27회, 거래량 부족 16회, 단독 VWAP 차단
  4회, 하락추세 3회다. 전자는 보유·정산 상태관측이고 나머지 23회가 실제 진입
  차단이므로 한 순위로 표시한 기존 경고는 원인 해석을 흐렸다.
- 저빈도 이벤트의 기존 `top_reasons`는 보존하면서 `top_entry_reasons`,
  `top_maintenance_reasons`, 양쪽 관측 수를 추가했다. journal과 Telegram도 진입차단과
  보유/정산관측을 분리한다.
- TV 스캔 이벤트는 기존 pool size와 원래 임계값만 저장해 실제 fallback 퍼널을
  복원할 수 없었다. 이제 1차·fallback 임계값/건수, 최소 목표, 선택 출처와 전체 심볼을
  구조화해 남긴다.
- 실시간 비교에서 상대거래량/최소 누적거래량별 후보는 1.8/50만 2개,
  1.08/50만 4개, 1.0/50만 5개, 0.8/25만 9개였다. 당일 Nasdaq은 약 -0.015%,
  `sideways|unknown|calm`이다. 평균 이하 활동까지 받아야 9개가 되므로 진입·스캐너
  임계값 완화와 stale 개장 풀 고정은 거부했다.
- 다음 3개 최종 Nasdaq 세션에서 작은 풀·1% 미만 주문률이 반복되고 차단 후보의
  15/30/60분 비용 후 성과가 양수일 때만 재검토한다. 정책평가 `id=117`에 근거,
  대안, 반증조건을 보존했다. 진단 회귀와 전체 **871개** 테스트(118.23초)가
  통과했다.
- 배포 전 온라인 백업은
  ...
  `quick_check=ok`, 외래키 위반 0, 생성 시 미종결 실행 0이다.
- 구현 커밋 `6393322`를 원격 `master`에 푸시하고 14:20:52 UTC에 서비스를
  PID 1881955로 재시작했다. `active/running`, `NRestarts=0`이며 자연 사이클 23회,
  API 263시도/263논리요청 전부 성공, 종단실패·회복재시도·미종결 실행·warning 이상
  journal·민감 URL/Bearer·유휴 DB/WAL FD가 모두 0이다.
- 배포 뒤 `tv_scan.id=17591`은 1차 1개→fallback 6개, `id=17598`은 1차
  1개→fallback 9개와 선택 심볼을 새 필드로 기록했다. 기준 1.8→1.08을 더 낮추지
  않고도 두 번째 자연 재스캔에서 최소 목표 9개를 회복했다.
- 배포 증거는 평가 113/114/117에 연결했고 `reviewed_at`은 비워 전향검증을 계속한다.
  후속 Telegram 보고는 `telegram_message_log.id=2904`, `success=1`이다.

## [2026-08-13] 미장 미체결 한도 반환과 제한적 빈도 회복

### 원인·성과 판단
- 8월 13일 미장 유일 검증주문 `TBBB` 17주는 정상 접수됐지만 0주
  체결로 34분 뒤 취소됐다. 기존 카운터는 이를 세션 1회 진입으로
  영구 계산해 이후 적격 신호도 막았다.
- 국장은 같은 날 10회 진입·청산했으므로 공식과 CB를 유지한다. 미장
  `VWAP+RSI`도 최근 확정 33청산·10승·평균 Net `-0.313%`·누적
  `-$703.97`이고 당일 차단신호 15/30/60분 평균도 모두 음수라 일반
  가드 해제와 정상 수량 확대는 거부한다.

### 수정
- 미장 `vps`의 정확한 `VWAP+RSI` 검증진입만 세션당 유효진입 3회,
  접수 6회, 일반 슬롯 10%로 조정한다. 최대 검증 배분은 일반 슬롯의
  30%이며 Nasdaq 비음수·600초 신선도와 모든 기존 위험조건을 유지한다.
- 제출 시도와 노출을 분리한다. 체결·열린주문·가상진입은 유효진입에
  포함하고, 0주 체결 종료는 진입 슬롯만 반환하면서 제출 횟수는
  유지한다. 현재 운영원장은 제출 1·유효진입 0·미체결종료 1이다.
- `/lab_guard`와 주문 context에 제출, 유효진입, 체결, 열림, 가상,
  미체결종료 및 양쪽 한도를 보존한다. 국장 정책 파일의 검증진입은
  계속 비활성 0/0이며 다른 미장 공식과 실계좌도 열지 않는다.
- 상세 근거·반증조건은
  `docs/TRADE_FREQUENCY_RECOVERY_2026-08-13.md`에 기록했다. 테스트,
  백업, 커밋, 재기동, 자연 주기와 Telegram 결과는 이어서 추가한다.

### 검증·배포 전 상태
- 시장별 설정, 제출/노출 원장, 미체결 슬롯 반환, admission, Telegram
  표시 관련 15개 테스트와 전체 **836개** 테스트(212.75초)가 통과했다.
  `compileall`과 `git diff --check`도 통과했다.
- 구조화 평가 `id=105`는 구현 검증 완료, 5개 검증청산·3개 최종
  Nasdaq 세션 전향검증 대기 상태로 남겼다. 온라인 백업은
  `data/trading_backup_20260813_180314_pre_bounded_frequency_recovery_deploy.db`,
  559,644,672바이트, SHA-256
...
### 실제 거래와 원인
- 국장은 하루 전체가 미거래가 아니었다. 매수접수 12건 중 10건 체결,
  미체결 2건 자동취소, 체결 포지션 10건 전부 청산됐다. 5승·비용 후
  `+8,050.97원`이며 마지막 신규진입은 10:26 KST였다.
- 10:42 KST 세 번 연속 비용 후 손실로 국장 CB가 발동했다. 당시 7청산
  누적은 `-4,458.52원`이었다. 11:12의 30분 쿨다운 해제 뒤에도 국장
  독립정책 `post_cb_max_fires_per_session=1`이 당일 일반 롱을 마감까지
  중단했다. 기존 보유 청산 3건은 계속 실행돼 `+12,509.49원`을 더했다.
- 기존 해제 알림은 `진입조건 재검사`라고만 했고, 14:58 저빈도 경고는
  실제 세션손실 한도를 버리고 `skip:no_action 50회`라고 표시했다.
  의도된 위험정지를 시스템 고장처럼 보이게 한 진단 오류다.

### 시장·성과 판단
- KOSPI는 `+3.5613%`, 거래량20일비 `1.1716`,
  `strong_up|normal|normal`로 마감했다. 최근 14일 국장 확정경계는
  118청산·승률 30%·평균 Net `-0.156%`·누적 `-168,830원`이다.
- 오늘 post-CB 차단 공식의 낙관적 최소비용 60분 반사실은 VWAP
  `-0.090%`, VWAP+VOL `-0.348%`, VOL `-0.413%`, RSI `-0.421%`다.
  30분도 모두 음수라 빈도 확대와 세션정지 해제를 거부한다.
- RSI 90 초과 단독 VOL 체결은 2세션 3건·누적 `+51,906.82원`이라
  `max_entry_rsi14`를 모든 독립 공식에 즉시 강제하지 않는다. 소표본
  선택편향 가능성을 명시하고 전략별 임계값 분리는 보류 기록한다.

### 수정
- 국장 무주문 결과가 active CB, 주문거부 CB, post-CB 세션정지,
  정확한 정책 WAIT, 준비신호 없음, 후보 없음 중 실제 원인을 보존한다.
- 50사이클 저빈도 알림은 구조화 원본 key는 유지하면서 사용자 문구만
  한국어로 표시한다. 오늘 경로는 `정책대기: 동일 시장 세션 반복손실
  한도 도달`로 남는다.
- 1회 발동 정책의 시간쿨다운 해제 이벤트에
  `session_entry_stop_active=true`를 저장하고, 일반 신규매수는 다음
  시장 세션까지 중단되며 기존 청산·인버스 감시는 계속됨을 알린다.
- 국장 `VWAP+VOL+RSI`, `VWAP+RSI` 가드는 관찰기간을 채워 해제됐고
  오늘 post-CB 상태도 다음 KRX 세션에는 이어지지 않는다. 국내는
  `VOL+RSI` 저성과 가드만 active다.

### 검증·배포 전 상태
- 리스크·알림·유동성 랩 관련 **316개** 회귀 테스트와
  전체 **835개** 테스트(208.13초), `git diff --check`가 통과했다.
- 구조화 평가 `id=104`는 구현 검증 완료, 다음 KRX 세션 초기화와
  3개 개입 세션 전향검증 대기 상태로 남겼다. 온라인 백업은
  `data/trading_backup_20260813_083028_pre_domestic_frequency_diagnostic_deploy.db`,
  559,616,000바이트, SHA-256
  `5bb2fdc5ef4ff62f104729ddc6b7b507e79663f77503435746c0f48a2e5b5dae`,
  `quick_check=ok`, 외래키 위반 0이며 평가 `id=104` 포함을 확인했다.
...
### 확인 결과
- 배포 뒤 텔레그램 `id=2239~2262`의 `watch:` 주문거부 18건은 실제
  주문이 아니었다. 후보가 정책상 WAIT인 상태를 일반 skip summary가
  `동작=주문거부`로 잘못 표시했다. 원인은 저빈도 진단 개선이 아니라
  기존 알림 분류가 내부 미실행과 증권사 거부를 구분하지 않은 데 있다.
- 실제 최신 거부는 국장 `360750` 1건이다. 35주 시장가 매도가
  `04:34:44Z` 접수·전량체결됐지만 체결조정 전 잔고가 9주/주문가능
  9주로 잠시 다시 보여 `04:37:06Z`에 재매도했고 KIS가
  `40240000 잔고내역 없음`으로 거부했다.
- 최근 7일의 같은 코드 3건을 전수 확인했다. 국장 `024840`은 전량매도
  63초 뒤, 미장 `MDLN`은 최종 정정매도 66초 뒤 같은 수량을 재매도해
  거부됐다. 모두 직전 주문은 뒤이어 전량체결로 확정돼 동일한
  잔고/체결원장 수렴 지연 문제다.

### 수정
- 국내·해외 매도 helper가 실시간 미체결 매도를 찾지 못한 경우에도
  최근 8분의 접수 미확정 매도 또는 최근 5분의 전량체결 매도가 현재
  표시 보유량을 덮으면 재주문을 보류한다. 양 시장의 주문가격·공식은
  건드리지 않았고, 브로커에 실제 미체결이 있으면 기존 45초 보호성
  정정 및 15분 일반 정정 규칙을 그대로 적용한다.
- `watch:`는 사이클/50사이클 저빈도 로그에는 보존하되 개별 거래알림은
  보내지 않는다. 내부 guard는 `매매미실행`, 실제 매수·매도 거부만
  `매수거부`/`매도거부`로 표시한다.
- 배포 뒤 `EGW00201` 실패시도 15건은 전부 재시도로 회복됐고 종단실패는
  0건이지만 2분 adaptive 창 종료 후 반복됐다. 최초 한도 응답 뒤
  950ms 응답완료 간격을 8시간 유지해 같은 VPS 세션 재발을 줄인다.

### 시장·성과 판단
- 8월 12일 Nasdaq은 `+0.5409%`, 거래량비 `0.8968`,
  `up|normal|calm`으로 마감했다. 첫 미장 `VWAP+RSI` 검증진입은 `SVV`
  75주, 총수익률 `+0.2323%`였지만 비용 후 `-$2.18135`였다. 한 세션
  한 건이라 기존 5세션/5청산 판정 기준을 바꾸지 않고 노출도 늘리지
  않는다. 상세는 `docs/ORDER_REJECTION_ANALYSIS_2026-08-12.md`에 남겼다.

### 검증·배포 전 백업
- 전체 **832개** 테스트가 216.26초에 통과했고 `compileall`,
  `git diff --check`도 통과했다. 구조화 평가 `id=103`은 구현검증 완료,
  배포·전향검증 대기 상태로 남겼다.
- 온라인 백업은
  `data/trading_backup_20260812_210736_pre_order_rejection_fix_deploy.db`,
...
### 현황과 원인
- 8월 11일 손실통제 배포 뒤 국장은 거래가 멈추지 않았다. 8월 12일
  KOSPI는 `+3.6799%`, 거래량비 `0.9819`,
  `strong_up|normal|normal`로 마감했고 14회 확정 진입·청산, 6승,
  비용 후 `+40,177.50원`이었다. 첫 전향 세션에서 KOSPI 0% 하한이
  유리한 장의 거래를 보존했으므로 국장 빈도 확대는 보류한다.
- 미장은 배포 후 신규 주문이 0건이다. 복합공식 네 개는 동적 가드,
  단독공식 세 개는 고정 가드로 차단돼 일반 롱 경로가 모두 닫혀 있다.
  다만 최근 차단 신호의 15/30/60분 비용 후 성과도 대부분 음수라
  전면 해제는 손실 빈도만 늘릴 가능성이 높아 거부한다.
- 저빈도 경고의 `no_overseas_candidate`는 후보풀이 빈 것이 아니라,
  후보가 모두 WAIT일 때 실제 사유를 버리는 집계 오류였다. 같은 시간
  TV 후보풀은 20종목 이상이었고 `volume_low`, `trend_down`, 정확한
  공식 가드가 계속 기록됐다. 정책차단 WAIT를 우선하고 그 외 대표
  WAIT 및 후보 수를 빈도 원인으로 남기도록 고쳤다.

### 과거 성공경로와 제한적 개선
- 7월 15일 이후 확장 `SELL_REAL` 179행은 정책 원장이 아닌 경로분석
  표본이다. `VWAP+RSI`는 66건·11승으로 수익 사례가 가장 많지만 평균
  비용 후 `-0.567%`, 누적 `-$2,430.83`라 일반 재개 근거는 아니다.
  수익은 진입수치보다 `take_profit` 7/7·평균 `+2.395%`와
  `time_exit_profit` 16/16·평균 `+0.581%` 청산에서 구분됐다.
- 미장 정책에만 `strategy_guard_probe`를 추가했다. 모의 `vps`, 당일
  Nasdaq 0% 이상, 지표 600초 이내, 정확한 `VWAP+RSI`, 세션당 1회,
  일반 슬롯의 10%로 제한한다. 다른 공식·실계좌·하락장·낡은 지표는
  계속 차단하고 모든 유동성·리스크·CB·포지션 조건은 유지한다.
- 제출은 `strategy_guard_probe_submitted`, 주문 사유, broker execution
  context에 시장 레짐과 함께 영구 기록한다. DB 세션 카운터로 재시작
  뒤에도 일일 한도를 유지하며 `/lab_guard`에서 사용량과 조건을 본다.
- 근거·반증·확대거부 기준은
  `docs/TRADE_FREQUENCY_ANALYSIS_2026-08-12.md`에 고정했다. 최소 5개
  확정세션의 5청산 전에는 유지/중단만 판단하고, 노출 확대는 최소
  10청산 전에는 검토하지 않는다.

### 검증·배포 전 백업
- 설정 독립성, `vps` 전용, 양의 신선 지수, 10% 수량, 세션당 1회,
  재시작 지속 카운터, 실계좌 차단, Telegram 표시, 실제 WAIT 진단을
  회귀 테스트로 고정했다.
- 최종 전체 **827개** 테스트가 207.68초에 통과했고 `compileall`,
  `git diff --check`도 통과했다.
- 구조화 평가는 국내 유지 `id=101`, 미장 전향검증 `id=102`로 남겼다.
  이를 포함한 온라인 백업은
  `data/trading_backup_20260812_174919_pre_us_guard_probe_deploy.db`,
  555,491,328바이트, SHA-256
  `e743062d8c1cfd7af36b1ac2a09aa192455973d0a74c10f6f1be74657c0bc8fe`,
  `quick_check=ok`, 외래키 위반 0이다. 커밋, 재시작, 자연 사이클,
  Telegram 보고는 아래 배포 후속 기록에 추가한다.
- 구현·분석 커밋 `6ba34f0`을 `/home/ubuntu/git_token.txt` 일회성
  credential helper로 원격 `master`에 푸시했다. 서비스를
  `2026-08-12T17:54:47Z`, PID `1694348`로 재기동했고
  `active/running`, `NRestarts=0`이다.
- 첫 세 자연 사이클은 미장 후보풀 20종목 이상을 평가했다. 기존
  `CCRN/NPAC` 청산감시는 유지됐고, 현재 신호가 유동성/단독공식 조건을
  통과하지 않아 신규 주문과 검증 제출은 각각 0건이었다. 이는 검증
  통로가 강제 주문이 아니라 기존 BUY의 가드만 제한적으로 우회함을
  확인한다.
- 배포 뒤 API 204회·논리요청 203건에서 `EGW00201` 실패시도 1회가
  기존 GET 재시도로 회복됐고 종단실패는 0이다. 첫 저빈도 진단은
  잘못된 후보없음 대신 `watch:[VWAP] standalone_vwap_blocked`를
  Telegram `id=2239`로 보냈다. 명시 배포보고 `id=2240`도 성공했다.
  `/lab_guard`는 검증진입 `vps전용(허용)`, `세션=0/1`, `슬롯=10%`,
  `지수하한=+0.00%`, `지표≤600초`를 정상 표시한다.
- 평가 `id=102` outcome에 커밋·백업·PID·자연사이클·API·Telegram을
  연결했고 `reviewed_at`은 비워 전향검증을 계속한다.

## [2026-08-11] 지속손실 재평가와 시장별 진입 중단식

### 전체기간 손실과 시장환경
- KIS 체결확정·세션소유 전략원장이 시작된 7월 27일부터 국장은
  133청산·30승·평균 Gross `-0.199%`·Net `-0.305%`, 누적
  `-367,497.69원`이다. 미장은 83청산·18승·평균 Gross `-0.073%`·
  Net `-0.575%`, 누적 `-$3,299.78/-4,454,699.23원`이다. 국장은
  12개 현지일 중 1일, 미장은 11개 중 0일만 비용 후 양수여서 이번
  거래만의 문제가 아니라 최근 계속된 구조적 음의 기대값으로 판정했다.
- 8월 7일 국장은 KOSPI `-0.597%`에서 `-29,641원`, 미장은 Nasdaq
  `+1.299%`에서 `-$212.69`; 8월 10일은 각각 `+0.653%/-4,597원`,
  `-0.319%/-$236.83`; 8월 11일 국장은 `+0.728%/-34,175원`이다.
  상승·하락 마감일 모두 손실이므로 마감 방향 하나로 양 시장을 같은
  방식으로 차단하지 않는다.
- 진입시점 KOSPI가 음수였던 일반 롱은 34청산·2승, 약
  `-128,882원`이었다. 8월 11일 KOSPI 약 `-1.03%`에서 1분 사이
  몰린 네 종목은 약 `-30,878원`을 잃었다. 반면 0%에서 시작한 당일
  첫 005930 수익은 보존한다. 미장 Nasdaq 진입구간은 음수·0~1%·
  1% 이상 모두 평균 Net 약 `-0.60%`라 방향 하한을 채택하지 않는다.

### 국장·미장 독립 개선
- 국장 일반 롱만 같은 KST 거래일 KOSPI 관측, 600초 신선도, 진입시점
  등락률 `0.0%` 이상을 요구한다. 인버스는 면제하고 exact benchmark
  shadow 경로를 유지한다. 전략가드는 생성 가능한 복합 라벨 전체와
  336시간·3청산·평균 Net `-0.30%` 기준을 쓴다. 운영 DB 복제검증에서
  `VWAP`, `VWAP+RSI`, `VOL+RSI`, `VWAP+VOL+RSI`만 차단되고 상대적으로
  나은 `RSI`, `VOL`, `VWAP+VOL`은 KOSPI 조건 아래 남는다.
- 미장은 방향 하한 없이 복합 라벨 누락을 모두 보완하고 336시간·
  3청산·평균 Net `-0.25%` 가드를 쓴다. 약 0.50% 왕복비용의 절반조차
  손실인 공식을 일시 중단한다. 복제검증에서 현재 네 복합공식 모두
  가드되며 기존 청산·인버스 shadow는 영향을 받지 않는다.
- 빈도 확대는 거부한다. 양 시장의 Gross 자체가 음수이고 미장은
  거래당 Gross-Net 차이가 약 0.502%p다. SEC와 FINRA도 거래별 비용과
  빈번한 일중거래 수수료가 수익을 낮춘다고 명시한다. 인버스 shadow도
  국장 5건 평균 `-0.614%`, 미장 3건 평균 `-0.990%`라 실거래 승격을
  거부한다.

### 로그·장애·검증
- 새 정책 SKIP 이벤트에는 당시 시장 레짐 전체를 저장한다. 이후
  15/30/60분 반사실을 방향·활동성·변동성별로 평가하고 5분 에피소드로
  중복을 줄인다. 상세 기준선과 반증조건은
  `docs/LOSS_ANALYSIS_2026-08-11.md`에 고정했다.
- 8월 11일 KIS 광범위 전송장애는 종단실패 60건을 만들었지만 미확정
  실행 0, 서비스 재시작 0이며 이후 회복했다. API pacing과 주문 POST
  재전송은 바꾸지 않는다. Telegram 502 한 건의 예외 URL에 인증정보가
  저장된 문제를 발견해 API 경계 마스킹과 기존 행 정리를 포함한다.
- 관련 회귀 367개에서 낡은 시장준비 전제 8건을 고친 뒤 재통과했고,
  전체 **825개** 테스트, `compileall`, `git diff --check`가 통과했다.
  로컬 개발 의존성에 `ruff`가 없어 별도 검사는 수행하지 않았다.
- Telegram 과거 오류의 URL·인증값은 메시지 원장 5개 셀에서 마스킹했고
  전체 TEXT 열 재검사 결과 실제 토큰 일치 셀은 0이다. DB
  `quick_check=ok`, 외래키 위반 0, 기한초과 미검토 정책평가 0이다.
  기존 평가 `4~7`은 인버스 shadow 유지와 빈도확대 거부로 종결했고,
  국장/미장 전향평가를 각각 `99/100`으로 등록했다.
- KIS 직접 조회는 국장 보유·당일 미체결 0, 미장 현지일 미체결 0이다.
  미장 기존 보유 `CCRN 281`, `NPAC 396`은 청산 감시를 유지한다. 새
  일반 롱 가드는 기존 포지션 청산을 막지 않는다.
- 마스킹·평가원장을 포함한 배포 전 온라인 백업은
...
### 누적 로그 재평가
- 배포 뒤 확정 일별총평은 국장 5세션, 미장 4세션 모두 비용 후 합계가
  음수였다. 빈도확대 평가 `10/11`의 WAIT 반사실에는 일부 양수 구간이
  있었지만 같은 레짐의 확정일이 최대 2일이라 요구한 3일 근거가 없다.
  따라서 진입 기준 완화는 계속 보류한다.
- 첫 CB 뒤 세션중단 평가는 국장 4개 확정 개입세션에서 60분 비용하한
  평균 `-0.079%`, 미장 3개에서 `-0.733%`였다. 국장은 2/4 세션만
  양수이고 미장은 0/3이므로 양 시장 모두 1회 중단을 유지하되 보편적
  우월성으로 해석하지 않는다.
- 2026-08-06 미장은 첫 Nasdaq 당일 관측치가 09:35 ET에 들어오기 전
  ZETA/SHOP이 09:31~09:32 ET 진입했고 합계 `-$236.04`로 청산됐다.
  이는 컨텍스트 저장 누락이 아니라 실제 장초반 데이터 준비 지연이었다.

### PDF 검토와 선택적 반영
- `How_to_Pick_Good_Stocks_SNULife_Edition.pdf` 35쪽 전체를 검토했다.
  가격 경로, 시장-섹터-종목 순서, 자금순환, 청산 유동성은 현재 시스템과
  접점이 있다. CEO 평가, 옵션, 월단위 박스권, 고정 시가총액 기준은
  보유기간·데이터·검증단위가 달라 직접 채택하지 않는다.
- 세부 채택·보류·반증조건은
  `docs/PDF_POLICY_REVIEW_2026-08-07.md`에 영구 기록했다. 산업 모멘텀
  연구는 섹터 기록의 시험 가치는 뒷받침하지만 현재 장중 후보풀 점수를
  주문 차단식으로 쓸 근거는 아니므로 그림자 기록만 시작한다.

### 시장별 구현
- 미장 일반 롱에 같은 뉴욕 현지일 시장관측치와 600초 신선도 조건을
  추가했다. 관측치가 들어오는 즉시 허용하므로 고정 장초반 금지가 아니며,
  전용 기준지수를 쓰는 인버스는 면제한다. 국장 스위치는 `false`다.
- TradingView 종목풀의 섹터·업종·등락·상대거래량 정보를 보존하고,
  미장 진입 시 선택 후보풀 안의 섹터 평균/중앙값/양수비율/종목 상대성과를
  기록한다. 국장은 KIS 업종명만 기록한다. 어느 쪽도 아직 주문을 막지
  않는다.
- 일별총평 품질원장과 전략리포트에 섹터 컨텍스트 보유·평가가능·롱정렬
  건수를 추가해 이후 레짐별 비용 후 성과와 연결할 수 있게 했다.

### 검증 상태
- 새 설정 독립성, 미장 missing/stale/fresh, 국장 비활성, 인버스 면제,
  섹터 계산과 원장 보존을 포함한 표적 30개 테스트가 통과했다.
- 핵심 회귀 495개 중 최초 494개가 통과했고, 테스트용 인버스 관측시각을
  잘못 미래로 둔 한 건을 수정했다. 이후 전체 **822개** 테스트,
  `compileall`, `git diff --check`가 통과했다. 국내·미장 실제 주문
  컨텍스트에 섹터 자료가 보존되는 테스트도 포함한다.

### 원장 정리·백업·배포
- 기한이 지난 빈도평가 `10/11`은 같은 레짐의 3개 확정세션에서 비용 후
  양의 WAIT 기회가 없으므로 `KEEP_THRESHOLDS`로 닫았다. 첫 CB 평가
  `93/94`도 국장 4개·미장 3개 성숙 개입세션 결과로 1회 중단을 유지한다.
  신규 미장 준비게이트 `96`, 국장/미장 섹터 그림자 `97/98`은 현지
  확정세션 전향검증으로 등록했다. 배포 시점 기한경과 미검토는 0건이다.
- KIS VPS 직접 점검은 국내 잔고·미체결 0, 해외 미체결 0이었다. 기존
  해외 보유 BZH 222·CCRN 281·NPAC 396의 청산 감시를 유지한다. BZH가
  NASD/NYSE 양쪽에 보이는 VPS 중복 노출은 합산하지 않는다.
- 변경 전 온라인 백업
  `data/trading_backup_20260807_174600_pre_pdf_market_sector_deploy.db`는
  489,844,736바이트, SHA-256
  `739d4161df6816f63a1a497d2f9e03b46ccd67ed959e9db919e1ccb94f279022`,
  `quick_check=ok`, 외래키 위반 0, 미확정 실행·가상 매도대기 0이다.
- 구현·근거 커밋 `6253d9a`를 `git_token.txt` credential helper로
  원격 `master`에 푸시했다. 서비스를 `2026-08-07T17:48:21Z`, PID
  `1631470`으로 재기동했고 `active/running`, `NRestarts=0`이다.
- 첫 자연 사이클은 TradingView 30종목 풀과 미장 감시를 정상 재개했다.
  확인 창 API 148회에서 실패 시도·종단 실패는 0이다. 시작/Scanner/
  휴장안내 Telegram `2075~2077`과 명시 배포보고 `2078`이 성공했다.
  배포 결과는 전향평가 `96~98`의 outcome에도 연결했다.

## [2026-08-04] 정책 전향검증을 현지 확정세션으로 구조화

### 확인된 기록 공백
- `market_session_reviews`는 최종 KOSPI·Nasdaq 환경과 비용 후 거래
  성과를 시장 현지일별로 계속 저장하고 있었지만,
  `policy_evaluation_log`의 검증조건은 자연어와 달력상
  `validation_due_at`뿐이었다. 따라서 휴장·지수 확정 지연·CB 미발동
  구간도 세 표본이 쌓인 것처럼 오해할 수 있었다.
- 정책평가 93/94의 실제 반증조건은 단순 경과일이 아니라, 기준 현지일
  뒤 CB 정책이 진입을 차단하고 60분 가격까지 성숙한 **각 시장의 최종
  세션 세 개**다. 현재 새 최종 세션은 0개이므로 정책 우열을 판정하지
  않는다.

### 구현과 독립성
- `policy_evaluation_log.validation_spec_json`을 추가해 기준 현지일,
  목표 최종세션 수, 차단사유, 에피소드 간격, 평가시계, 가격 허용오차,
  주문가능 세션 프로필을 기계 판독 가능하게 보존한다. 기존 행은 빈
  명세로 호환하며 93/94에는 서로 독립된 명세를 넣는다.
- 전략리포트는 시장별로 기준일 뒤 `확정경과`, 실제 `CB개입`, WAIT와
  주문 직전 SKIP을 합친 `차단` 세션·에피소드, 60분 `성숙` 세션,
  최소 왕복비용 후 Net·양수 세션 수, 차단일 레짐을 구분한다. 반복
  스캔은 동일 종목·현지일에서 5분 간격 에피소드로 축약한다.
- KRX는 정규장을 한국 현지 09:00~15:30으로 정의하고 Nasdaq도 자체
  거래시간·휴장 일정을 운영하므로, UTC/KST 달력 날짜를 공통 표본으로
  쓰지 않고 기존 KST/New York 귀속 함수를 그대로 사용한다.
  ([KRX Trading](https://global.krx.co.kr/contents/GLB/06/0602/0602010201/GLB0602010201T1.jsp),
  [Nasdaq Market Schedule](https://www.nasdaq.com/market-activity/stock-market-holiday-schedule))

### 검증과 판정 경계
- 저장·갱신·JSON 복원을 포함한 저장소 테스트와, 두 최종 국장 세션의
  WAIT/SKIP 중복 제거·CB 개입·60분 비용차감·레짐 연결을 검증하는
  분석 테스트를 추가했다. 전체 **817개** 테스트, `compileall`,
  `git diff --check`가 통과했다.
- 이 변경은 진입·청산·CB 수치나 거래 빈도를 바꾸지 않는다. 세 성숙
  최종세션 전에는 `관찰계속`, 기한이 먼저 오면 `inconclusive`로
  연장 검토하며 표본 0을 성공으로 바꾸지 않는다.

### 백업·배포와 실제 출력
- 변경 전 온라인 백업
  `data/trading_backup_20260803_180435_pre_policy_validation_progress_deploy.db`
  의 SHA-256은
  `f50c0f48bd0b6dbfc3ce019b0e9a048fd002df6d9e61071be1a71f5e1aca22b1`,
  무결성 `ok`, 외래키 위반 0, 정책평가 94행, 일별총평 157행,
  미확정 실행 0행이다.
- 구현 커밋 `8c48e96`을 원격 `master`에 푸시하고 서비스를
  `2026-08-03T18:05:44Z`, PID `1583863`으로 재기동했다.
  `active/running`, `NRestarts=0`이며 초기 API 86회에 실패·종단실패가
  모두 0이었다. 시작·Scanner Telegram `1824/1825`와 명시 배포보고
  `1826`이 성공했다.
- 실 DB 전략리포트는 93/94 모두 `확정경과=0`, `CB개입=0`,
  `차단=0세션/0ep`, `60m성숙=0`, `관찰계속(0/3세션)`으로 출력했다.
  재시작 첫 자연 사이클에서는 TGTX가 미장
  `post_cb_session_loss_limit_reached`로 차단됐다. 이는 뉴욕 8월 3일
  배포 당일의 부분세션이라 원 로그에는 남지만 `기준>2026-08-03`인 세
  전향 최종세션에는 포함하지 않는다.
- 구조화 정책평가 `id=95`는 이 측정 개선의 대안·반증조건·배포결과를
  보존한다. 직접 소형모델 A/B는 수행하지 않았으므로 비교가치는 계속
  `unverified`다.

## [2026-08-04] 첫 CB를 시장별 당일 일반 롱 손실예산으로 고정

### 배포 후 타임라인 재검증
- 8월 3일 미장 첫 CB는 `13:50:19Z`, 두 번째는 `15:21:10Z`에
  발동했다. ITGR 진입이 두 번째 뒤였다는 초기 의심은 타임라인 대조로
  기각했다. 첫 CB 해제 뒤 FERG 2회와 ITGR 2회가 진입했고, 두 번째 CB
  뒤 신규 매수는 0건이므로 기존 2회 게이트 자체는 정상 작동했다.
- 첫 CB 뒤 확정된 FERG 2건과 ITGR 1건은 모두 비용 후 손실,
  합계 `-276,410.64원`이다. 두 번째 ITGR 66주는 아직 보유 중이고
  검토 시점 Gross `+0.06%`로 미장 최소 왕복비용 `0.50%`를 넘지
  못한다. 당시 Nasdaq은 `+1.30~+1.78%`로 상승 중이어서, 광역지수
  회복만으로 종목·전략 손실 군집이 해소됐다고 볼 수 없다.
- 국장은 첫 CB 뒤 19청산 중 1승, `-65,603.24원`이었다. 이 근거는
  미장과 독립적으로 첫 CB 이후 일반 롱 재진입의 손실예산을 소진했다.

### 시장별 정책 수정과 반대 증거
- 국장과 미장의 `post_cb_max_fires_per_session`을 각각 2에서 1로
  낮춘다. CB 활성 중에는 기존 차단이 작동하고, 30분 뒤 해제돼도 같은
  시장 현지일의 일반 롱은 다시 열리지 않는다. 시장별 인버스 shadow는
  계속 면제돼 하락장 관찰 경로를 막지 않는다.
- SEC의 시장접근 위험관리 설명은 자동 주문 오류가 빠르게 누적될 수
  있어 사전 설정한 자본·활동 한도와 자동 차단을 두는 원칙을 설명한다.
  이는 이 개인 시스템에 규제가 직접 적용된다는 주장이 아니라, 반복
  손실 뒤 같은 자동 경로를 계속 허용하지 않는 위험관리 참고 근거다.
...
- 반대 증거도 보존한다. 기존 두 번째 CB 뒤 차단 신호의 60분 낙관적
  비용하한 반사실은 한 확정 급등일 `VWAP+RSI +0.683%`, 한 확정
  급락일 `-2.135%`, 현재 임시 급등일 `-1.220%`로 방향이 일관되지
  않다. Heston 등은 단기 반전이 일시적 유동성 불균형과 스프레드에
  좌우될 수 있음을 보여 주므로, 한 급등일의 회복을 보편적 재진입
  우위로 해석하지 않는다.
  ([Heston, Korajczyk and Sadka](https://doi.org/10.1111/j.1540-6261.2010.01573.x))

### 기한경과 평가 부채 정리 기준
- 정책평가 25, 28, 34, 35, 57, 59~63, 84의 기한이 지났지만 결과가
  구조화 원장에 귀환하지 않았다. FSUN/HUBB는 모두 체결 0·취소로
  종결됐고, 폐장 조정은 다음 주문가능 창에 재개됐으며, 가상 정산대기는
  0건으로 해소됐다. 인버스는 국장 3건·미장 3건이 모두 음수라 live
  승격은 계속 기각한다.
- 운영 DB는 약 398MB, 검증 백업은 94개·24.15GB, 파일시스템 여유는
  149GB다. 삭제·VACUUM·WAL 변경 근거는 아직 없지만 백업 증가를 계속
  추적한다. 가상 미장 비용 후 성과도 음수이고 실제 양 시장 계좌 성과도
  음수라 빈도 확대 보류를 유지한다.
- 각 만기행은 `confirmed`, `superseded`, 또는 자연표본 부족을 명시한
  `inconclusive` 결과로 닫고 원 증거를 삭제하지 않는다. 미장
  `VWAP+VOL` 가드 자체는 손실 회피 방향이 맞았지만 48시간 보존창은
  조기 해제를 허용해 정책평가 91의 168시간 창으로 대체됐다.

### 전향 반증
- 첫 CB 뒤 차단 신호를 15/30/60분으로 계속 평가한다. 각 시장에서
  최소 세 최종 세션 동안 비용 후 양의 차단 기회가 회피손실보다 크고,
  실제 첫 CB 뒤 재진입 손실 군집이 재현되지 않으면 2회로 되돌리거나
  회복조건을 전략별로 세분화한다.
- 첫 CB 이전 진입, 기존 보유의 청산, 인버스 shadow, 다음 현지 세션 중
  하나라도 잘못 막히면 즉시 범위를 되돌린다. 거래 빈도 감소 자체는
  실패가 아니며, 비용 후 기회손실이 검증될 때만 실패로 판정한다.

### 검증·원장 정리·배포 전 상태
- 양 시장 설정 독립성, 첫 CB 한 건 뒤 국장·미장 일반 롱 차단, 다음
  현지일 해제와 인버스 면제를 포함한 표적 6개 및 저장소 전체
  **816개** 테스트, `compileall`, `git diff --check`가 통과했다.
- KIS를 `17:43:57Z/17:44:19Z`에 두 번 조회했다. 매회 국내 잔고 0,
  양 시장 미체결 0이며 미장 실제 보유는 중복 거래소 응답을 합쳐
  CCRN 281·ITGR 66·NPAC 396이다. 세 거래소 잔고와 주문이력은 모두
  1페이지다. ITGR가 VPS `NASD/NYSE` 양쪽에 노출되는 기존 중복은
  수량을 합산하지 않고 종목·거래소 정규화 경계로 처리한다.
- 변경 전 온라인 백업
  `data/trading_backup_20260803_174443_pre_single_cb_session_stop_deploy.db`
  의 SHA-256은
  `67cf485f4a998622a99911c0cedc9769be299fa7e54f1e35350c484343d6901c`,
  무결성 정상, 외래키 위반 0, 미확정 실행 0이다.
- 기한경과 정책평가 11건과 부분 대체된 `id=90`에 실제 결과를
  기록했다. 자연 교차 위험일 지연체결이 없던 `35`, 자연 다중페이지가
  없던 `60`, 임계표본 미달 인버스 `57`은 `inconclusive`; 48시간
  가드창 `84`는 `91`로 `superseded`; 나머지는 각 원장의 구체적
  결과로 확인했다. 기한경과 미검토는 0건이다.
- 새 정책평가 `id=93`은 국장, `id=94`는 미장 첫 CB 세션 손실예산을
...
- 로컬 GPT/Codex 및 Claude JSONL, Git 이력, 운영 DB, 서비스
  저널, 기존 두 문서를 대조했다. 사용자 상시 지시와 빠졌던 결정,
  원본 로그 경로, 향후 검증 항목은
  `docs/SESSION_LOG_RECONCILIATION_2026-08-03.md`에 이관했다.
- `POLICY_EVALUATION.md`에는 8월 1일 결정과 배포 결과가 있었지만
  구조화 `policy_evaluation_log`는 7월 29일 `id=88`에서 멈췄고,
  `WORKLOG.md`에도 8월 실행이 없었다. 운영 DB 백업 후 8월 1일
  누락 평가와 이번 평가를 구조화 `id=89~92`로 복구했다.
- 기존 `market_regimes`와 관측 원장, BUY 실행 컨텍스트에는 KOSPI·
  Nasdaq 지수와 진입 당시 시장상황이 정상 보존됐다. 다만 최종 시장
  성격과 당일 거래 성적을 결합한 고정 일일 총평이 없어 원시 테이블을
  매번 재계산해야 했다.

### 시장별 일일 총평과 기록 품질
- 새 `market_session_reviews` 원장은 시장·현지 거래일별 최종 지수
  OHLC/수익률/거래량비/범위/레짐과 세션소유 KIS 체결확정 진입·청산,
  비용 후 손익, 전략·청산사유 분해를 함께 저장한다.
- 진입 실행의 `entry_market_regime` 컨텍스트 존재율과 현지 거래일
  일치율도 저장한다. 최종 레짐 수집 시 해당일을 갱신하고 서비스 시작
  시 과거 최종일을 백필하므로, 지연 체결 보정 후에도 재계산할 수 있다.
- 분석 보고서에는 최신 양 시장 총평 품질과 구조화 정책평가 원장의
  최신 번호·기한경과 미검토 건수를 노출한다. 계산 코드는 10,532줄인
  `liquidity_lab.py`를 더 키우지 않고 `market_review.py`에 분리했다.

### 8월 3일 국장 정책
- KOSPI는 6,257.45, `-5.1247%`, 20일 거래량비 `0.6347`, 장중범위
  `5.64%`, `strong_down|quiet|calm`으로 마감했다. 현지일 최종
  원장은 20진입·22청산·1승·`-189,636.80원`이다. 개장 직후 전일
  보유 2건 청산을 분리하면, 첫 CB 뒤 새로
  진입·청산된 19건은 1승, 합계 약 `-65,603.24원`이었고 모든 진입이
  KOSPI `-3.45~-5.39%` 구간에서 발생했다.
- 국장에만 CB 발동 뒤 KOSPI 회복선 `-3.0%`와 거래일당 최대 두 번
  발동 한도를 적용한다. 일반 급락장 전체 롱 차단은 아니며 CB 이전과
  인버스 shadow에는 적용하지 않는다. 미장 설정도 바꾸지 않는다.
- 15/30/60분 차단기회 반사실을 보존하고, 이후 최소 세 최종 국장
  세션에서 회피손실보다 양의 차단기회가 크고 반복손실도 줄지 않으면
  임계값을 축소하거나 철회한다.

### 8월 3일 미장 정책과 빈도 판단
- 검토 시점 Nasdaq은 약 `+2%`였지만 최종 마감 전이고 KIS 거래량이
  0이라 활동성은 `unknown`이다. 당일 성과의 최종 레짐 귀속은 확정행
  뒤 수행한다. 네 확정 청산은 모두 비용 후 손실, 합계 약
  `-280,844원`이었고 기존 미장 CB 두 번 한도가 이후 롱을 막았다.
- 미장 `VWAP+VOL` 가드는 48시간 손실 표본이 먼저 만료된 뒤 최종
  세션 수만 채워져 해제됐고, 재개 뒤에도 손실이 이어졌다. 미장만
  전략가드 관찰창을 48시간에서 168시간으로 늘린다. 국장은 48시간을
  유지한다.
- 최근 14일 계좌 청산은 국장 95건·승률 21%·`-276,195원`, 미장
  69건·승률 26%·`-2,844,012원`이다. 비용 후 양의 기대값이 없으므로
  진입 빈도 확대를 거부한다. 완료된 인버스 shadow 6건도 전부 음수라
  실거래 승격을 거부한다.

### 검증 상태와 남은 운영절차
- 집중 6개와 저장소 전체 **815개** 테스트, `git diff --check`가
  통과했다. 직접 소형모델 A/B는 수행하지 않았으므로 고문맥 추론의
  비교우월성은 계속 `unverified`다. 다만 누락된 구조화 평가와 일별
  총평 부재를 발견한 가치는 변경 없음 기준선 대비 별도로 검증한다.
- KIS를 16초 간격으로 두 번 조회해 국내 잔고 0, 당일 미체결 0,
  `0162Z0` 보유 0, 주문 38515의 과거 체결량 0을 확인했다. VPS 과거
  행은 취소확정 없이 잔량 73을 계속 반환하므로 이를 숨기지 않고
  `audited_session_expiry_not_broker_cancel_flag` 근거와 두 스냅샷을
  실행 원장·이벤트에 보존했다. 실행 292는 `CANCELED`, 체결 0,
  취소수량 73으로 종결됐고 미확정 실행은 0건이다.
- 변경 전 온라인 백업
  ...
  무결성 정상, 외래키 위반 0이다. 정책평가 `id=89~92`와 최종 시장
  총평 157행을 구조화 복구했다.
- 백필 과정에서 기존 8월 1일 문서의 “7월 31일 미장 12청산”이 뉴욕
  현지일이 아니라 KST 날짜 집계였음도 확인했다. 뉴욕 현지 7월 31일은
  2청산·2패·`-1,459,481.84원`이며, 12청산·`-1,738,865.34원`은
  뉴욕 7월 30일 꼬리와 31일 초반을 섞은 KST 운영일 값이다. 이후
  최종 레짐 평가는 시장 현지일 원장만 사용한다.
- 뉴욕 7월 31일 진입환경 품질은 `1/2(50%)`다. RBLX 진입은 당시
...
### 발견과 공식 확인
- 운영 DB의 미장 가상 포지션 `CPRX` 118주는 평단
  `$31.48`, `2026-07-15 05:55:58 KST` 진입 뒤 약
  14일 동안 남아 있었다. 현재 원장에는 10,135개
  사이클 관측, 가격 범위 `$31.48~31.49`, 최대
  `hold_cycles=51,989`가 쌓였고 `time_exit_cost_floor_hold`
  로 반복 평가됐다. 실제 KIS 잔고에는 CPRX가 없었다.
- KIS 직접 조회는 현재가 `$31.49`, 거래량 0,
  `매도불가`, 7월 15일 이후 일봉 거래량 0, 현재 분봉 0행을
  반환했다. 이 패턴만으로 상장폐지를 추정하면 거래정지·휴장·
  저유동성 종목을 오인할 수 있으므로 자동 휴리스틱은 쓰지
  않는다.
- Nasdaq 공식 기업행동 공지는 합병이 7월 15일 개장 전에
  종결됐고, 7월 14일 장후 약 19:50 ET부터 거래정지,
  최종거래일 7월 14일, 7월 16일 거래정지 효력, 주당 현금
  대가 `$31.50`를 명시한다.
  ([Nasdaq Trader](https://www.nasdaqtrader.com/TraderNews.aspx?id=ECA2026-485))
  SEC 공시도 합병 효력 시 보통주가 이자 없는 주당 `$31.50`
  현금 수령권으로 전환된다고 확인한다.
  ([SEC](https://www.sec.gov/Archives/edgar/data/1369568/000119312526210587/d90722ddefa14a.htm))

### 미장 전용 정책과 정산 경계
- `overseas.json`에 공식 출처를 포함한
  `corporate_actions.CPRX` 현금합병 정의를 추가했다.
  설정 로더는 미지원 유형·상태, 알 수 없는 필드, 비정상 ISO
  날짜, 최종거래일/효력일 역전, 0 이하 대가, 잘못된 통화와
  URL을 거부한다. 국장 맵은 비어 있고 두 시장의 맵 객체도
  독립적이다.
- 사이클에서 먼저 KIS 잔고를 새로 받은 뒤에만 기업행동을
  조정한다. 캐시가 현재 사이클 것이 아니거나 상장 거래소
  잔고가 조회되지 않았으면 보류한다. 실제 CPRX 수량이
  하나라도 있으면 자동정산하지 않고 증권사 현금입금·세금·
  잔고변경 대조를 요구하는 이벤트와 텔레그램 알림을 남긴다.
  기존 `virtual_sell_pending`이 있어도 자동정산하지 않는다.
- 실제 보유가 없는 순수 가상 포지션만 공식 `$31.50`로
  한 번 전량 정산한다. CPRX의 총 현금전환 손익은
  `(31.50-31.48)*118 = +$2.36`이다. 이는 전략 청산이
  아니고 일반 미국주식 매매 수수료 모델도 설명하지 못하므로
  `excluded_from_performance=1`과
  `corporate_action_cash_settlement` 사유를 기록한다.
- 정산 뒤 가상 포지션, 신호·실패·대기·청산가드 캐시,
  전략 관리자와 저장된 진입·가격·스냅샷 상태를 평탄화한다.
  효력 발생 뒤 비보유 CPRX는 호가 API 호출 전에 제외하며,
  주문 직전에도 `corporate_action_effective`로 재차 막는다.
  같은 정산은 포지션 삭제를 멱등성 키로 삼아 중복 매도를
  만들지 않는다.

### 기각한 일반화와 빈도 판단
- 이 사례는 `time_exit_cost_floor_hold`가 틀렸다는 증거가
  아니라 거래 가능한 주식이 현금 수령권으로 바뀐 사실을
  시스템이 몰랐던 수명주기 누락이다. 따라서 모든 종목의
  절대 최대 보유시간을 새로 두거나 비용 하한 청산을 완화하지
  않는다.
- 거래량 0·동일가격 반복으로 기업행동을 추정하는 방식,
  CPRX 행을 수동 삭제하는 방식, 실제 보유까지 자동 현금정산하는
  방식은 각각 오탐·감사손실·브로커 원장 불일치 때문에
  기각했다.
- 죽은 종목의 반복 호가·차트 호출과 오래된 슬롯 점유를
  제거하면 정상 후보가 쓸 수 있는 운영 여유는 회복된다.
  그러나 현재 합산 포지션은 한도보다 낮았고 최근 미장 비용 후
  기대값도 음수이므로 진입 문턱이나 동시주문 한도를 풀어
  매매 빈도를 인위적으로 늘리지는 않는다.

### 검증과 전향 반증
- 잘못된 설정 거부, 시장 맵 독립성, 성과 제외 매도,
  최신 무실보유 잔고 정산, 실보유 보존, 오래된 잔고 보류,
  호가 전 제외, 주문 전 차단, 국장·일반종목 무영향과 중복
  정산 방지를 회귀 테스트로 고정했다. 저장소 전체
  **808개** 테스트, `compileall`, `git diff --check`가
  통과했다.
- `policy_evaluation_log.id=88`에 공식 근거, 선택·기각
  대안, 성과 제외 이유와 반증조건을 기록했다. 직접 소형모델
  A/B는 수행하거나 추정하지 않았다. 아래 운영 검증은 변경
  없음 기준선이 놓친 폐장 수명주기 지연을 찾아 수정한 가치를
  확인했지만 모델 크기 자체의 우월성은 주장하지 않는다.
- 중복 매도, 실제 보유 자동정산, 공식값과 다른 대가, 효력 전
  또는 일반종목 차단, 국장 영향, 기업행동 손익의 전략성과
  포함 중 하나라도 나타나면 자동정산을 중단하고 되돌린다.
  배포 전 공식 미체결 2회 확인과 백업을 거쳐 자연 사이클에서
  CPRX 1회 정산·성과 제외·텔레그램 보고를 검증했다.

### 첫 배포에서 발견한 폐장 수명주기 지연
- 구현 커밋 `bbbacdb`를 원격 `master`에 먼저 푸시하고,
  공식 KIS 미체결을 두 번 조회해 매회 국장 0·미장 0건을
  확인했다. 사전 백업
  `data/trading_backup_20260729_221554_pre_corporate_action_deploy.db`
  는 SHA-256
  `12faee9936a9278c861fec8aa32a211556429d23d0488aba2a7dfb945f2895e9`,
  무결성 정상, 외래키 위반 0이며 CPRX 118주와 정책평가
  88번을 보존한다.
- 서비스를 `2026-07-29T22:16:11Z`, PID `1496765`로
  재시작했다. 첫 자연 사이클 `20205`는
  `last_error=None`, 신규주문 없이 완료됐으나 KST 07시
  폐장 구간의 `no_supported_market_open` 조기 반환 때문에
  API 0회였고 CPRX도 정산되지 않았다.
- 기업행동은 거래 신호가 아니라 자산 수명주기 조정이므로 다음
  장까지 기다릴 이유가 없다. 다만 기존 폐장 API 0회 원칙을
  넓게 깨지 않도록, 효력 발생한 가상 기업행동 포지션이 실제로
  남은 경우에만 폐장 사이클에서 KIS 거래소별 잔고를 한 번
  대조한다. 정산으로 포지션이 사라지면 다음 폐장 사이클은
  다시 API 0회다. 이 배포 관측으로 발견한 경계를 별도 회귀
  테스트로 고정한다.

### 후속 통합배포와 멱등성 확인
- 폐장 예외를 포함한 전체 **808개** 테스트, `compileall`,
  `git diff --check`를 통과한 후속 커밋 `43a0555`를
  `git_token.txt`의 값을 노출하지 않는 credential helper로
  원격 `master`에 푸시했고 전체 SHA가 일치했다.
- 두 번째 재시작 전에도 10초 이상 간격의 공식 KIS 미체결
  조회를 두 번 수행해 매회 국장 0·미장 0건을 확인했다.
  백업
  `data/trading_backup_20260729_222443_pre_closed_market_corporate_action_deploy.db`
  는 SHA-256
  `f62c83ffafcb74286d1dba713e61e9a2389fc6829f12ec480fbf1d980c3034dd`,
  무결성 정상, 외래키 위반 0이며 정산 직전 CPRX 118주를
  보존한다.
- 서비스를 `2026-07-29T22:24:52Z`, PID `1497465`,
  `NRestarts=0`으로 재시작했다. 첫 새 자연 사이클
  `20210`은 NASD·NYSE·AMEX 잔고를 각각 한 번 조회했고
  세 호출 모두 1차 성공, HTTP 200, 종결실패·재시도 0이다.
  실 CPRX가 없음을 확인한 뒤 118주를 공식 `$31.50`로
  정산해 가상주문 `id=255`, 손익 `+$2.36`,
  `excluded_from_performance=1`로 저장했다.
- `virtual_corporate_action_settled` 이벤트 `id=4939`에는
  Nasdaq·SEC 근거, 수량·평단·현금대가·실보유 0·성과 제외를
  보존했다. 저장 종목상태는 보유 0, 마지막가격·진입가격·
  진입시각·고점·신호 스냅샷 모두 `NULL`이고 CPRX 가상
  포지션은 삭제됐다. 기업행동 텔레그램 `id=1409`도
  성공했다.
- 후속 폐장 사이클 `20212`는 `last_error=None`, API 0회로
  복귀했다. CPRX 포지션 0, 매도 1, 정산 이벤트 1,
  기업행동 알림 1로 중복 실행이 없다. 첫 배포 발견 보고
  `id=1410`, 후속 성공 보고 `id=1411`도 모두 성공했다.
- 정책평가 88번은
  `confirmed_material_lifecycle_fix_vs_unchanged_policy_no_direct_model_ab`
  로 갱신했다. 운영 검증이 변경 없음 기준선의 실제 누락을
  찾았다는 비교가치는 확인됐지만, 직접 소형모델 A/B는 하지
  않았으므로 별도 모델 결과를 추정하지 않는다. 일반
  `time_exit_cost_floor_hold`, 국장·미장 진입식, 동시주문
  한도와 매매 빈도는 그대로 유지한다.

## [2026-07-30] 미장 청산 후행성과의 정식 레짐 평가

### 운영 상태와 표본 경계
- 국장 VWAP 배포 뒤 CBZ가 실제 잔고 111주인데 감시 상태는
  `virtual_sell_pending`, 보유수량 0으로 보이는 현상을
  조사했다. 정규장 실매수 111주가 확정된 뒤 모의계좌 주문
  불가 시간에 가상 청산해 손익시점만 고정했고, 동일 111주를
  다음 주문가능 세션에 실정산하도록 의도적으로 예약한 상태다.
  미체결 실주문은 없고 중복 매도는 정산대기 행이 막는다.
- 전체 `trend_filter_lost` 29건은 국장 2건을 포함한다. 미장
  KIS 체결확정·세션소유 전략청산은 27건이며, 7월 27~29일
  세 개 날짜에 걸쳐 있다. 최종 레짐별로는
  `sideways|normal|normal` 10건·2일,
  `strong_down|normal|high` 17건·1일이다.

### 청산 지연 가설 평가
- 급락 레짐 17건을 5/15/30/60분 늦췄을 때 목표시각 직전
  관측가와 실제 청산가의 평균 차이는
  `-0.163%/-0.299%/-0.197%/-0.137%`다. 기존 청산이 네
  구간 모두 추가 하락을 피했다.
- 보합 레짐 10건은 `-0.047%/-0.110%/+0.021%/-0.061%`다.
  30분 양수폭은 0.021%뿐이고 관측률도 3/7, 43%라 청산
  지연 근거가 아니다. 60분은 다시 음수다.
- 따라서 `min_hold_before_trend_exit` 상향과 청산 빈도
  축소는 기각한다. 급락 레짐은 한 최종 세션, 보합 레짐은 두
  최종 세션뿐이므로 어느 쪽도 3개 최종 세션 기준을 충족하지
  않는다. 기존 정책을 유지하고 다음 세션을 관찰한다.

### 재현 가능한 평가 기능
- `summarize_exit_forward_performance`와
  `/lab_report exit-forward [시간]`,
  `scripts/analyze_trades.py --exit-forward-hours N`을 추가했다.
  세션소유 KIS 체결확정 `SELL_REAL`만 포함하고, 같은 시장·
  종목·현지 거래일의 주문가능 시간대만 비교한다.
- 각 5/15/30/60분 목표시각보다 늦은 가격을 보지 않고,
  목표 직전 5분 안의 마지막 관측만 사용한다. 청산 뒤 별도
  관측이 없으면 청산가를 0%로 재사용하지 않고 `관측누락`으로
  남긴다. 이는 후보가 스캔에서 사라진 경우를 인위적인 중립
  성과로 섞지 않기 위한 경계다.
- 지연차이는 같은 매수·매도 비용을 공유하는 두 청산시점의
  증분 비교라 왕복비용을 다시 차감하지 않는다. 음수면 기존
  청산이 추가 하락을 회피했고, 양수면 지연 청산 가격이
  우월했다는 뜻이다. 정책 재검토 기준은 동일 최종 레짐
  최소 5청산·3거래일이다.
- 분석 함수·텔레그램 명령·기존 WAIT 선행성과를 묶은 표적
  3개와 관련 파일 전체 161개 회귀가 통과했다. 반증조건은
  세 최종 세션 이후 여러 구간에서 충분한 관측률로 지연차이가
...
### 성과·시장 레짐 근거
- KIS 체결확정·세션소유·실거래 기준 국장 단독 `VWAP` 청산은
  23건, 5승, 비용 후 합계 `-126,272.58원`, 진입원금 대비
  건별 평균 Net `-0.6203%`다. 이미 동적 손실가드는 최근
  3건 평균 Net `-1.6504%`로 2026-07-29 22:13 KST부터
  활성 상태지만, 최소 3개 최종 세션 뒤 해제되면 같은 약한
  진입이 다시 열릴 수 있었다.
- 가드 활성 직전 2026-07-29 오전의 `006340`, `229200`,
  `233740` 세 단독 VWAP 청산은 각각 `-14,022.98원`,
  `-8,297.33원`, `-22,015.05원`, 합계 `-44,335.36원`이다.
  진입 스냅샷을 현재 공통 진입식으로 재평가하면 거래량 비율이
  `0.0600/0.6287/0.2825`라 모두 `volume_low`다.
  `229200/233740`은 분봉 단기선이 장기선보다 낮고 모멘텀과
  마지막 봉 수익률도 모두 음수였다.
- 그날 KOSPI는 `5,663.24`, 전일 대비 `-5.9834%`, 20일
  거래량비 `1.0338`, `strong_down|normal|extreme`으로
  마감했다. 다만 KIS 공식 지수 분봉 API
  `FHKUP03500200`의 현 응답으로 09:11~09:15 KST 구간을
  복원하지 못했으므로 진입 당시 정확한 지수 수준은 추정하지
  않는다.
- 급락장이라는 사실만으로 모든 롱 진입을 막지는 않는다.
  같은 2026-07-29 미장 `strong_down|normal|high`에서
  `VWAP+RSI`는 4건·3승·평균 Net `+0.2594%`였다. 시장
  레짐은 성과의 조건부 설명변수로 기록하되, 하루의 광역지수
  방향을 종목 상대강도보다 우선하는 전면 차단 근거로 쓰지 않는다.

### 국장 전용 변경
- `domestic_momentum_v4`에
  `entry_confirmation_strategy_flags=["VWAP"]`을 추가했다.
  국장 단독 VWAP 매수는 기존 전략 신호뿐 아니라 공통
  모멘텀 진입식도 `ready`여야 한다. 감시 상태 생성, 최종
  후보 선정, 주문 직전의 세 경로가 같은 검사를 수행하고,
  약한 신호는 `strategy_confirmation_<reason>`으로 원장에
  남는다. 미장 목록은 비워 기존 미장 공식에는 변화가 없다.
- `233740`은 삼성자산운용 공식 설명대로 코스닥150 일별
  수익률의 2배를 추구하는 레버리지 ETF이므로
  `leveraged_etf_symbols`에 추가했다.
  ([KODEX 코스닥150 레버리지](https://www.samsungfund.com/etf/product/view.do?id=2ETF56))
  이 상품의 확인 청산은 4건·2승·합계 `-5,482.83원`,
  평균 Net `-0.1049%`뿐이라 후보 확대 근거는 부족하다.
  따라서 상품 분류와 동적 풀 승인을 별도 목록으로 분리하고,
  `dynamic_pool_approved_leveraged_symbols=["122630"]`은
  유지했다. `233740`은 보유·명시 후보로 들어올 때 이중
  추세 보호를 받지만 자동탐색 풀에는 새로 편입되지 않는다.
- 레버리지·인버스 ETP는 대체로 일일 목표로 재설정되며,
  보유기간과 변동성에 따라 누적 성과가 목표 배수와 크게
  달라질 수 있다는 일반 위험도 이 보수적 분리와 일치한다.
  ([FINRA](https://www.finra.org/investors/insights/lowdown-leveraged-and-inverse-exchange-traded-products))

### 기각한 대안·반증조건
- `strong_down` 당일 전체 국장 롱 차단은 표본이 한 레짐에
  집중되고 종목 상대강도 기회를 지울 수 있어 기각했다.
  거래 빈도 확대도 단독 VWAP의 음의 Net 기대값과 비용 회전을
  늘리므로 기각했다. 현재 인버스 정책은 상품정확 기준지수로
  정렬한 이후 진입 표본이 0건이어서 계속 shadow로 둔다.
- 이후 `strategy_confirmation_*` 대기 신호의 15/30/60분
  비용 후 반사실을 최종 시장 레짐별로 쌓는다. 세 개 이상
  최종 세션에서 차단 기회비용이 회피 손실보다 커지거나,
  확인된 VWAP 진입까지 막혀 시장별 거래기회가 구조적으로
  사라지거나, 미장·인버스·청산 경로가 영향을 받으면 범위를
  축소하거나 되돌린다.

### 검증과 장기 개선 기준
- 설정 독립성, 저거래량 VWAP의 감시·주문 이중 차단,
  확인된 VWAP 통과, `122630/233740` 레버리지 보호,
  구조화상품 동적 풀 비확대를 포함한 표적 7개와 확장
  277개 테스트가 통과했다. 구형 설정의 레버리지 승인 폴백
  회귀를 더한 저장소 전체 **794개**, `compileall`,
  `git diff --check`도 통과했다.
- 사용자 정정에 따라 장기 개선 할당량은 10만 토큰이 아니라
  **1만 크레딧**으로 기록한다. 할당 크레딧 초기화 전까지
  평가·공식 지식검색·검증·수정을 이어가고, 초기화가 확인되면
  장기 goal을 종료한다. 서비스 재시작 변경은 매번
  `git_token.txt` 인증 Git 푸시와 텔레그램 보고를 동반한다.

### 통합배포와 사후 확인
- 구현·근거 커밋 `82853ff`을 `git_token.txt`의 값을
  노출하지 않는 임시 credential helper로 원격 `master`에
  푸시했고 로컬·원격 전체 SHA가 일치했다. 판단과 대안,
  반증조건은 `policy_evaluation_log.id=86`에 저장했다.
  고비용 추론의 우월성은 추정하지 않고
  `deployed_forward_counterfactual_pending_no_direct_model_ab`
  로 두었다.
- KIS 미체결을 10초 간격으로 두 번 조회했다. 매번 미장
  주문이력 5페이지 전체와 국장 1페이지를 확인했고 양 시장
  모두 0건이었다. 이후 온라인 백업
  `data/trading_backup_20260729_211743_pre_domestic_vwap_confirmation_deploy.db`
  을 만들었다. SHA-256은
  `8bc3d05197c6b8b73c60836c9365ad19fbbdbc1297580a5c942946e367b5e8c7`,
  무결성 정상, 외래키 위반 0이다.
- 서비스를 `2026-07-29T21:17:55Z`, PID `1489006`,
  `NRestarts=0`으로 재시작했다. 런타임은 국장 확인
  `["VWAP"]`, 미장 `[]`, 국장 레버리지
  `["122630","233740"]`, 동적 승인 `["122630"]`을
  읽었다. 첫 자연 사이클 `20128`은
  `2026-07-30 06:19:37 KST`, `last_error=None`으로
  끝났고 신규 주문은 없었다.
- 배포 뒤 첫 완료 시점의 API 96회는 시도 실패·최종 실패·
  재시도가 모두 0이다. 텔레그램 시작 알림 `1399/1400`과
  명시 배포보고 `1401`도 모두 성공했다. 국장이 닫혀 있어
  자연 `strategy_confirmation_*` 표본은 아직 없으며 다음
  정규장에서 레짐별 반사실을 수집한다.
...
### 세 번째 확정 세션과 비용 후 성과
- 2026-07-29 16:05 ET 뒤 KIS 최종 재조회에서 NASDAQ
  Composite는 `-1.7445%`, `strong_down|unknown|high`로
  확정됐다. 지수 누적거래량은 아직 0이라 활동성은
  `unknown`으로 남기고 기존 10분 재조회 정책을 유지한다.
  방향과 변동성은 확정됐지만 활동성 조건별 결론은 내리지 않는다.
- 최근 48시간 미장 `VWAP+VOL`은 KIS 체결확정·세션소유
  청산 20건, 비용 후 승리 2건이다. 평균 Gross는
  `+0.0284%`였지만 평균 Net은 `-0.4737%`, 합계는
  `-$596.841786`, `-805,736.39원`이다. 약 0.5% 왕복비용이
  미세한 총수익 우위를 완전히 소진했다.
- 세션별로도 7월 27일 1건 평균 Net `-0.3918%`, 28일
  6건 `-0.6243%`, 29일 13건 `-0.4105%`로 세 확정
  세션이 모두 음수다. 27·28일은
  `sideways|normal|normal`, 29일은
  `strong_down|unknown|high`라 한 급락 레짐에만 생긴
  손실도 아니다.

### 미장만의 제한적 변경
- 미장 `overseas_momentum_v2`의
  `strategy_guard_strategy_flags`에 정확한 복합 라벨
  `VWAP+VOL`만 추가한다. 국장 목록은 `VWAP/RSI/VOL`로
  유지한다. `VWAP+RSI`는 같은 48시간 8건·5승·평균 Net
  `+0.5537%`이므로 차단하지 않는다.
- 이는 공식 삭제나 임계값 재최적화가 아니라 기존 동적
  손실가드의 관측 범위 확장이다. 롤링 48시간 표본이
  3건 미만이 되거나 평균 Net이 `-0.3%` 위로 회복해도,
  활성 시점부터 미장 최종 세션 3개를 관측한 뒤에만
  자동 해제한다. 다른 복합 라벨은 세션 수가 부족해 편입하지
  않는다.
- 거래 빈도 확대는 거부한다. 현재 문제는 기회 부족이 아니라
  총 기대수익이 거래비용을 넘지 못한 것이다. 같은 공식을 더
  자주 실행하면 관측된 음의 Net 기대값과 비용 회전만 늘어난다.

### 반증조건
- 다음 평가에서는 가드로 차단된 `VWAP+VOL` 신호의 15/30/60분
  사후 Net 반사실과, 가드 해제 뒤 실제 체결을 세션별로 비교한다.
  차단 반사실이 세 최종 세션 이상에서 비용 후 양수이고 기회비용이
  회피손실보다 커지거나, 국장·다른 복합공식·청산 경로까지
  차단되면 범위를 축소하거나 되돌린다.

### 구현·검증·통합배포
- 미장 전용 설정 커밋 `2309a93`을 `git_token.txt` 인증으로
  원격 `master`에 푸시했다. 전체 **786개** 테스트,
  `compileall`, `git diff --check`가 통과했다. 정책 판단과
  반증조건은 `policy_evaluation_log.id=84`에 기록했고,
  차단 신호의 선행성과가 쌓일 때까지 추론가치는
  `unverified_forward_counterfactual_pending`으로 유지한다.
- 공식 KIS 당일 미체결을 10초 간격으로 두 번 조회해 모두
  0건임을 확인한 뒤 온라인 백업
  `data/trading_backup_20260729_201435_pre_exact_inverse_and_us_vwap_vol_guard_deploy.db`
  을 만들었다. SHA-256은
  `a9941376de8e5a3f768a43d582f3668296ee1f31386bbef7dff541fd5fdb2368`,
  무결성 정상, 외래키 위반 0이다.
- 서비스를 `2026-07-29T20:14:44Z`, PID `1480573`,
  `NRestarts=0`으로 재시작했다. 첫 전체 사이클은
  `20:16:57Z`, `last_error=None`으로 완료됐고 배포 뒤
  API 115회는 시도·종결 실패 모두 0, 신규 주문도 0이다.
- `strategy_guard_state`에는 미장 `VWAP+VOL`이
  `20:15:13Z`, 롤링 경계 안 19건·평균 Net `-0.4780%`로
  활성화됐다. 배포 전 평가의 20건과 차이는 시간이 지나
  48시간 경계의 첫 행이 빠진 결과다. 첫 사이클의 무보유
  후보는 선행 `volume_low`에서 멈춰 자연 가드 차단 표본은
  아직 없으며, 기존 보유 CBZ·NPAC·CCRN·CPRX는 계속
  `time_exit_cost_floor_hold`로 평가돼 보유·청산 경로가
  가드에 오염되지 않았다.
- 텔레그램 배포보고 `telegram_message_log.id=1394`는
  성공했다. 다음 최종 미장 세션부터 실제 차단 후보와
  15/30/60분 비용 후 반사실을 원장에 연결한다.

## [2026-07-30] 국·미장 인버스 상품 기준지수 정합성 분리

### 발견한 정책 오염
- 시장 성격을 평가하는 광역 기준은 국내 KOSPI, 해외 NASDAQ
  Composite로 유지하는 것이 맞지만, 인버스 상품의 진입 여부까지
  같은 지수로 판단한 것은 상품 계약과 맞지 않았다. `114800`과
  `252670`은 F-KOSPI200 일간 수익률을, SQQQ는 Nasdaq-100,
  SPXU는 S&P 500, SOXS는 NYSE Semiconductor Index를 각각
  추종한다.
- 기존 shadow 원장에는 국내 `114800` 1건이 KOSPI 기준
  `-0.928%`, 해외 SQQQ·SPXU 2건이 NASDAQ Composite 기준
  각각 `-1.160%`, `-0.788%`로 종결돼 있다. 거래 결과 자체는
  유효하지만 상품의 실제 기준지수 정책을 검증한 표본은 아니므로,
  이를 새 공식의 수익성 근거로 합치면 안 된다.
- 실제 KIS 조회에서 같은 시점의 NDX와 SPX가 서로 다른 등락과
  변동성 레짐을 보였다. 광역 NASDAQ 하나로 두 상품을 함께
  활성화하면 SQQQ와 SPXU의 계약상 다른 위험을 지워버린다.

### 시장·상품별 소유권
- 시장 일일 성격과 일반 전략 성과 결합은 계속 KOSPI와 NASDAQ
  Composite를 쓴다. 별도 `inverse_benchmark_regimes`와
  append-only `inverse_benchmark_observations`가 인버스 상품
  활성화 기준을 소유한다.
- 국내 `114800`, `252670`은 KIS 파생 일봉
  `FHKIF03020100`, 연속선물 별칭 `101000`의
  F-KOSPI200을 공유한다. 응답의 실제 결제월 코드, 전일 종가,
  현재가, 등락률이 서로 맞지 않으면 저장하지 않는다.
- 해외 SQQQ는 `NDX`, SPXU는 `SPX`를 각각 수집한다. KIS가
  제공하는 PHLX Semiconductor `SOX`는 SOXS의 계약상
  NYSE Semiconductor Index와 같지 않으므로 대용하지 않는다.
  정확한 런타임 소스를 확보할 때까지 SOXS는
  `inverse_exact_benchmark_unavailable`로 닫힌 방향 차단한다.
- 정책 JSON과 저장 키는 모두 시장별로 소유한다. 현재 임계값과
  shadow 공식은 기존 값을 복제했지만 이후 표본·변경은 국장과
  미장, 그리고 상품 기준지수별로 독립 평가한다.

### 시간경계·성과 비교가능성
- KOSPI200 선물 정규장은 현물보다 늦은 15:45 KST에 끝난다.
  처음의 현물 15:35 최종 처리안은 종가 단일가 10분을 누락하므로
  공식 KRX 거래시간 대조 뒤 15:50부터만 최종 레코드로 고정했다.
  장중에는 5분 간격으로 잠정값을 갱신한다.
- 새 shadow 진입은 기준 코드·소스가 설정과 정확히 일치할 때만
  `benchmark_alignment_version=product_exact_v1`을 동결한다.
  저장소와 `/lab_performance`는 상품정확 표본과 과거 대용지표
  표본을 동시에 보이되, 정책 승률·평균손익 판단에는 전자만
  사용한다. 과거 행은 수정하거나 삭제하지 않는다.
- 상품별 레짐 관측 이벤트도 성과 보고에 포함한다. 광역시장
  관측만 표시돼 실제 NDX/SPX/SOXS 차단 이유가 숨는 문제를
  함께 수정했다.

### 공식 검증·실데이터·반증조건
- 상품 계약은 삼성자산운용 KODEX 상품 페이지, ProShares
  SQQQ/SPXU, Direxion SOXS 공식 페이지로 확인했다. KIS 공식
  저장소의 국내 파생 일봉 계약과 실 API 응답도 대조했다.
- 운영 DB에 쓰지 않는 임시 저장소 실검증에서 `101000`은
  `A01609`로 해석돼 2026-07-29 F-KOSPI200 `-6.07%`,
  NDX는 2026-07-29 `-1.3866%`, SPX는 `-1.1653%`로
  저장됐다. 국내 최신일은 현재 KST 날짜보다 이전이라 당일
  append-only 관측을 만들지 않았고, NDX/SPX는 각각 한 건을
  만들었다.
- 설정·저장소·수집기·shadow·텔레그램 관련 **472개** 테스트,
  전체 **786개** 테스트, `compileall`, `git diff --check`가
  통과했다. 배포 후 자연 관측 결과는 같은 항목에 이어 기록한다.
- 반증조건은 상품 설정과 저장 기준 코드·소스 불일치, KIS
  연속코드가 실제 결제월을 반환하지 않음, 등락률·현재가
  불일치, 15:45 이전 국내 최종 고정, SOXS의 유사 지수 대용,
  또는 과거 대용지표 손익이 상품정확 집계에 섞이는 경우다.
  하나라도 발생하면 신규 인버스 활성화를 중단하고 수집 계약을
  재검증한다. shadow-only와 라이브 하드블록은 유지한다.

### 통합배포 자연 검증
- 정확 기준지수 구현 커밋 `d9c8bfe`를 원격 `master`에
  푸시했다. 첫 새 사이클은 F-KOSPI200 `101000`
  `-6.0700%`, NASDAQ-100 `NDX` `-2.0560%`, S&P 500
  `SPX` `-1.5161%`를 각각 2026-07-29 최종행으로 저장했다.
  SQQQ와 SPXU는 각자의 정확 지수로
  `strong_down|unknown|high` 활성화됐고, SOXS는
  `event_log.id=4852`에서
  `inverse_exact_benchmark_unavailable`로 닫힌 방향
  차단됐다.
- `/lab_performance 48` 재현에서 기존 국내 1건·해외 3건은
  모두 `과거대용지표(평가제외)`로 표시되고 상품정확 종료
  표본은 0건으로 분리됐다. 따라서 기존 NASDAQ Composite
  shadow 손실이 새 NDX/SPX 공식의 승률과 평균손익에 섞이지
  않는다.
- 정책 판단은 `policy_evaluation_log.id=83`에 기록했다.
  고맥락 공식자료 교차검토가 기존 광역 대용 설계에서 놓친
  상품 계약과 KRX 15:45 시간경계 불일치를 실제로 찾아낸
  비교가치는 배포 자연경로까지 확인했다. 직접 소형모델 A/B는
  수행하지 않았으며 그 한계도 outcome에 남겼다.

## [2026-07-29] 미장 동일 세션 반복 차단 뒤 일반 롱 중단

### 두 번째 해제 뒤 즉시 재노출된 위험
- 미장 비용차감 3연속손실 차단은 같은 뉴욕 거래일에
  `16:02:45Z`, `17:20:44Z` 두 번 발동했고 각각 약 30분 뒤
  해제됐다. `18:10:52Z` 평가시점에 첫 발동 뒤 확정된 전략
  청산은 11건이며 모두 비용 후 손실이었다. 합계 Net `-$404.175456`,
  `-545,636.85원`, 개별 수익률 범위는 `-0.4050%`부터
  `-1.3653%`다.
- 두 번째 해제 뒤 NASDAQ가 `-0.5368%`로 기존 회복선을
  통과하자 일반 롱 슬롯이 즉시 다시 채워졌다. 회복선은
  급락상태 지속을 거르는 역할은 했지만, 같은 세션에서 이미
  두 번 확인된 전략·비용 환경의 반복 실패까지 멈추지는
  못했다.
- 이 11건은 한 세션에 상관된 표본이므로 진입공식 영구삭제나
  임계값 최적화의 근거로 쓰지 않는다. 반대로 두 번의 독립된
  3연속손실 발동은 그 세션의 일반 롱 위험노출을 더 늘리지
  않을 충분한 실시간 위험 신호로 본다.

### 시장별·세션 한정 방어
- 미장 정책에만 `post_cb_max_fires_per_session=2`를 둔다.
  같은 뉴욕 거래일의 영속 `cb_fired(type=consecutive,
  market=overseas)`가 두 번이고 실제 해제도 같은 세션이면,
  다음 세션까지 일반 롱 신규진입을
  `post_cb_session_loss_limit_reached`로 중단한다.
- 인버스 후보와 기존 포지션 청산은 기존 공식 경계에서
  면제한다. 따라서 하락장 역방향 정책의 관측·진입과 손실
  축소를 위한 청산은 계속 작동한다. 다음 뉴욕 거래일에는
  자동 해제된다.
- 국장은 `post_cb_max_fires_per_session=null`로 유지한다.
  같은 날 국장에는 반복 발동 근거가 없으므로 미장 결과를
  복제하지 않는다. 공통 일일손실 한도와 시장별 3연속손실
  30분 차단도 변경하지 않았다.

### 구현·검증·반증조건
- 설정 스키마와 시장별 JSON을 분리 확장하고, 재시작 후에도
  이벤트 원장에서 시장 현지 세션 날짜·발동시각·이벤트 ID를
  복원한다. 운영 DB 읽기 재현은 미장 세션 `2026-07-29`,
  발동 2건(`event_log.id=4622,4710`)을 정확히 반환했다.
- 회복 게이트는 동일 세션 2회 한도를 NASDAQ 회복조건보다
  먼저 검사한다. 일반 롱은 차단하고 인버스는 허용하며,
  다음 세션과 국장은 영향받지 않는 회귀를 추가했다.
- 설정·메시지·유동성 랩 전체 관련 테스트 **275개**가
  93.14초에 통과했다. 저장소 전체 테스트 **772개**도
  178.39초에 통과했고 `compileall`, `git diff --check`도
  정상이다.
- 수익성 우월성은 아직 미검증이다. 다음 3개 최종 미장
  세션에서 반복 발동 횟수, 차단된 일반 롱의 사후 성과,
  허용된 인버스 성과를 기록한다. 두 번째 발동 뒤 차단한
  반사실의 비용 후 기대값이 양수이거나, 한도 때문에 유효한
  롱을 반복적으로 놓쳐 회피손실보다 기회비용이 커지면
  `2`를 완화하거나 shadow로 되돌린다.

### 배포와 자연 차단 검증
- 구현 커밋 `2d5cbd9`를 `git_token.txt` 인증으로 원격
  `master`에 푸시했다. WSO 장기미체결 자동취소와
  SITE·FER·MOD 교체청산을 포함해 최근 주문 0건을 10초
  간격으로 두 번 확인한 뒤 온라인 백업
  `data/trading_backup_20260729_184143_pre_post_cb_session_fire_limit_deploy.db`
  을 만들었다. SHA-256은
  `5b571debb9824c5482a12041737d31f1df71052fa073d00f21bf6c298d055388`,
  무결성 정상, 외래키 위반 0이다.
- 배포 대기 중 두 번째 해제 뒤 새로 진입한 일반 롱 6건
  PAYO·FEIM·MOD·FER·ALOY·SITE가 모두 비용 후 손실로
  종결됐다. 합계 Net `-$270.404745`, `-365,046.41원`이며
  `18:30:46Z`, `event_log.id=4784`에서 세 번째 미장
  차단이 발동했다. 새 규칙의 동일 세션 반사실은 이 여섯
  진입을 막지만, 독립 세션 수익성 증거로 보지는 않는다.
- 서비스를 `18:42:22Z`, PID `1468526`, `NRestarts=0`으로
  재시작했다. `cb_state_restored event_log.id=4802`가 미장
  활성 차단과 `19:00:46Z` 해제 예정시각을 복원했고, 당일
  확정 위험손익은 총 `-649,611.98원`, 미장
  `-576,853.72원`으로 재조정됐다.
- 첫 사이클은 `18:44:09Z`, `last_error=None`으로
  완료됐다. API 110회는 시도·종결 실패 모두 0이고 신규
  주문도 0이다. PAYO 일반 롱 후보가
  `cycle_log.id=226697`,
  `post_cb_session_loss_limit_reached`로 자연 차단됐다.
  텔레그램 배포보고 `telegram_message_log.id=1387`도
  성공했다.
- `policy_evaluation_log.id=82`의 고맥락 검토 비교가치는
  기존 NASDAQ 회복선만으로는 놓친 반복 발동 위험을 찾아
  자연 차단까지 검증했다는 범위에서 `confirmed`로
  갱신했다. 작은 모델 통제 A/B는 수행하지 않았고, 2회
  임계값의 독립 세션 수익성 우월성은 계속 미검증이다.

## [2026-07-29] 미장 연속손실 차단 후 NASDAQ 회복 게이트

### 30분 해제 뒤 같은 하락상태 재진입
- 미장 3연속 비용차감 손실로 `16:02:45Z`에 발동한
  서킷브레이커는 정책대로 `16:33:32Z`에 해제됐다.
  직전 NASDAQ 잠정 수익률은 `-1.0494%`였고, 4초 뒤
  LXFR 440주가 재진입해 `16:48:55Z`에 Net
  `-$37.700455`, `-0.5021%`로 청산됐다.
- FVRR도 NASDAQ `-1.2235%` 관측 뒤 진입했다. 지수가
  `16:53:07Z`에 `-0.9866%`로 회복된 뒤 ASH, FBRX,
  MAS가 진입했고, 이후 MAS와 EVR 손실까지 이어져
  `17:20:44Z`에 두 번째 미장 차단이 발동했다.
  고정 30분은 시간을 보장했지만 손실 원인이 된 시장상태의
  변화를 보장하지 않았다.
- 새 기준의 읽기 전용 반사실 적용에서는 LXFR와 FVRR만
  회복 전 대기, `-1.0%` 위로 올라온 뒤의 7개 진입은
  허용된다. FVRR는 645주 부분체결 뒤 원주문을 취소하고
  잔량 73주를 교체해 총 718주가 평균 `$9.35796657`에
  확정됐으며 Net `-$46.713882`, 약 `-0.6938%`다. 따라서
  회복선 아래에서 진입한 LXFR와 FVRR 두 건은 모두 비용 후
  손실이지만 같은 잠정 세션의 사후 반사실이므로 수익성 개선이나
  최적 임계값을 주장하지 않는다.

### 시장별 회복조건과 주문 경계
- 미장 정책에만 `post_cb_reentry_benchmark_floor_pct=-1.0`,
  `post_cb_reentry_regime_max_age_sec=600`을 둔다. 같은 뉴욕
  거래일에 시간차단이 해제된 뒤, 10분 이내 NASDAQ 관측이
  `-1.0%` 이상이어야 일반 롱 신규진입을 재개한다. 관측이
  없거나 오래됐으면 닫힌 방향으로 대기한다.
- `-1.0%`는 한 경로에 새 숫자를 맞춘 값이 아니라 기존
  미장 인버스 레짐 경계와 일치시킨 값이다. 인버스 후보와
  기존 포지션 청산은 면제한다. 국장은 차단 해제 뒤 신규
  진입이 없었고 당시 KOSPI 장중 원장도 없으므로 필드를
  `null`로 두어 미장 근거를 복제하지 않았다.
- 감시상태 생성, 후보 선택, KIS 실주문 제출 직전, 가상주문
  기록 직전에 같은 공식 가드를 검사한다. 같은 사이클 안에서
  새 CB 해제가 발생하면 해제시각이 바뀐 캐시는 즉시
  무효화해 첫 해제 사이클 우회를 막는다. 자동해제 알림도
  무조건 `매수 재개` 대신 `시장별 진입조건 재검사`로
  표현한다.

### 전략가드 판단 철회와 관측성 보강
- 해외 `VWAP+VOL` 48시간 성과는 13건, Net 승리 2건,
  평균 Net `-0.4774%`로 임계치를 넘었다. 하지만 완전일치
  감시목록은 결함이 아니라 복합공식을 최소 3개 최종
  NASDAQ 세션 전까지 영구·동적 차단으로 넓히지 않기로 한
  선행 결정이었다. 현재 확정 세션은 7월 27·28일 두 개뿐이라
  전체 공식 감시 패치는 테스트 뒤 철회했고 배포하지 않는다.
- `/lab_guard`는 이런 손실 공식을 단순 `참고` 대신
  `참고(범위밖·임계초과)`로 표시한다. 정책은 바꾸지 않되
  세 번째 최종 세션 평가 후보가 보고서에서 숨지 않는다.

### 근거·검증·반증조건
- Moreira와 Muir의 변동성 관리 연구는 높은 변동성에서
  위험노출 축소가 위험조정 성과를 개선할 수 있음을 보였지만,
  이번 `-1.0%` 숫자나 단일 세션 반사실을 검증하지는 않는다.
  따라서 기준은 서킷브레이커가 이미 발동한 같은 세션에만
  한정하고 일반 진입공식·포지션 상한·거래 빈도를 완화하지
  않는다.
- 설정 분리, 회복 전/후, 관측 누락·노후화, 다음 세션 해제,
  국장 비활성, 인버스 면제, 새 해제시각 캐시 무효화,
  감시·선택·실주문·가상주문 재검사를 회귀로 고정했다.
  집중 39개 테스트와 추가 경계 6개가 통과했고, 전체
  **770개** 테스트도 최종 176.53초에 통과했다. `compileall`과
  `git diff --check`도 정상이다.
- 온라인 백업은
  `data/trading_backup_20260729_173929_pre_post_cb_regime_gate.db`,
  SHA-256
  `5fe8129cfd2238548c3e5659a3bad90b23c9c6f062e4911e84dbd079c06b05e2`다.
  무결성 정상, 외래키 위반 0이며 당시 FVRR 73주 청산
  잔량이 남아 있어 재시작은 보류했다.
- 판단은 `policy_evaluation_log.id=81`에 고맥락 검토,
  비교기준 `30-minute time-only overseas breaker release`,
  추론가치 `unverified`로 먼저 기록했다. 청산 주문 종료와
  배포 자연경로는 아래 결과로 이어서 검증했다.
- 반증 조건은 새 해제 첫 사이클 우회, NASDAQ 회복 후
  오차단, 10분 초과 관측으로 진입, 인버스·청산 차단,
  미장 조건의 국장 전파다. 수익성 판단은 최소 차단 5건과
  허용 5건이 각각 종결되고 3개 최종 미장 세션이 쌓인 뒤
  수행한다.

### 배포와 첫 회복경로 자연검증
- 구현 커밋 `70a95dd`를 `git_token.txt` 인증으로 원격
  `master`에 푸시했다. FVRR 교체주문과 뒤이어 발생한 FBRX
  80주 청산이 모두 확정된 뒤 배포경계 온라인 백업
  `data/trading_backup_20260729_175206_pre_post_cb_regime_gate_deploy.db`
  을 만들었다. SHA-256은
  `c1611e60d54feee7a8270e2cde96ac6b61b1dc3dc2a5c3cc97d70bec0d55d378`,
  무결성 정상, 외래키 위반 0이다.
- 서비스를 `2026-07-29T17:52:10Z`, PID `1463854`,
  `NRestarts=0`으로 재시작했다. 두 번째 미장 브레이커의
  실제 해제시각 `17:51:28Z`, 시장별 스트릭 0, 당일 확정
  위험손익 `-309,062.54원`이 그대로 복원됐고
  `last_error=None`이다.
- 첫 배포 사이클의 NASDAQ 관측은 `17:52:15Z`,
  `-0.5368%`였다. 해제 뒤 약 47초, 회복선보다
  `+0.4632%p` 위인 신선한 관측이어서 WSO 20주와 TEVA
  162주 일반 매수가 각각 `17:53:58Z`, `17:54:03Z`에
  제출됐다. TEVA는 즉시 162주 전량이 평균 `$34.64`에
  확인됐다. 이는 회복 후 허용 경로를 검증하지만 두 진입의
  향후 손익은 아직 정책 성과 표본으로 종결되지 않았다.
- 다음 자연 사이클에서 ASH 92주 `trend_filter_lost` 매도도
  `17:55:17Z`에 제출돼 기존 포지션 청산 면제가 확인됐다.
  재기동 뒤 첫 194개 논리요청은 시도·종결 실패 모두 0이고,
  텔레그램 배포보고 `telegram_message_log.id=1362`도
  성공했다.
- `policy_evaluation_log.id=81`은 배선·복원·회복 후 허용과
  청산 면제를 구현 확인으로 갱신한다. 고맥락 검토가 고정시간
  기준이 놓친 시장상태 문제와 두 실제 손실 경로를 연결한
  추가가치는 확인됐지만, 작은 모델과의 통제 A/B는 수행하지
  않았고 `-1.0%`의 수익성 우월성도 표본 기준 충족 전까지
  미검증이다.
